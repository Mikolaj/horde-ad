...
a fixed roster --- `micro-regime3`'s A/A controls measure that separately
and find it benign.
...
from the victim's very first sample (criterion per-iteration OLS slopes at R2 >=
...
short-lived per-position gather-slice vectors of 3600 and 10368 bytes --- all
just above GHC's 3276-byte large-object threshold, so each occupies its own
1--3-block group outside the nursery and dies at the next GC.  Interleaved
with only 9.4 MB/iteration of ordinary heap churn, this checkerboards
the megablocks.  During ~7 s of the benchmark the RTS pool grows steadily
from 1117 to 2180 MiB (RSS timeline: growth from t~1.8 s, plateau by t~8.4 s)
and then never shrinks or defragments: `-I0` prevents idle collections, major
GCs do not rebuild free-list structure, and `-Fd1` changes neither the residency
(page-fault counts identical to +/-1) nor the effect.  The poisoning saturates:
one such predecessor produces the full effect, and all four `two-gathers`
variants together add nothing (13.12 vs 13.14 ms).  Below saturation
it is dose-dependent: a 5 s predecessor leaves a 2043 MiB pool and a 12.73 ms
victim.
...
us/iteration in both contexts, allocation is byte-identical (83.2076 MB), GC
...
allocation splits some other group --- and the recycled working set cycles
through hundreds of MB before reuse: ~14 MB/iteration (223k lines x 64 B)
that L3 used to retain round-trips to DRAM.  The nursery, by contrast,
is no stable contiguous region and confers no such protection: at `-A` >= 16m
it is silently chunked into 4 MB chunks (overriding even an explicit `-n`),
every out-of-line allocation permutes its block chain and nothing re-sorts it,
and post-GC trim and grow refill it with singleton blocks drawn
from the scattered free lists (RTS-source reading; GHC's own Note [Sources
of Block Level Fragmentation] documents the decay and names, without
implementing, periodic nursery reallocation) --- and the small-pinned churn
condition in the [micro-regime3 section](#micro-regime3) below is the measured
case of ordinary-allocation-heavy code paying for degraded block-level layout.
...
the allocation between sprays 4.7x and the iteration time 4x --- a ~4x denser
checkerboard laid over ~4x more iterations per criterion budget.  Measured
...
sides, common-mode), 0.98 alone --- reproducing the retracted A/B's 1.200 /
0.995 / 0.98 triple.
...
profile --- the fused-gather case --- the same roster poisons one build
and not the other.  The red flag is a predecessor whose per-iteration allocation
...
(ordinary-allocation-heavy) phase of the process +12--17% at `-A32m`
and +29--44% of true steady state at `-A64m`--`-A1G` --- larger
than this document's 22% --- where a matched-count spray of own-group objects
taxes a few percent at most, and this document's own 3600 B class taxes
that reproducer's ordinary-churn victim zero.  Distinct, because
the discriminators split: mem-in-use barely moves where the pool here doubles,
and `-H2G` does not cure it where here it does --- though the counter signature
...
`perf stat` (hardware counters are blocked for unprivileged sessions
on this machine by `perf_event_paranoid`).  Criterion's GC placement (major per
