...
Filed as [GHC work item 27667](https://gitlab.haskell.org/ghc/ghc/-/work_items/27667); this file stays as the filed record, the text from "## Summary" down being the filed body. Title: **Recompilation checking ignores every setting that carries a value: `-fllvm`, the inliner thresholds, `-fproc-alignment` and the `-pgm*` and `-opt*` families are all missed**. The prose is ASD-STE100 Simplified Technical English. The case that found the defect is a cache-line alignment measurement in the orthotope micro-benchmark, on the unmerged `speedup-strided-tovector` branch, which is not public; the filed body is self-contained. The absence of loop alignment that the case turns on is [work item 27668](https://gitlab.haskell.org/ghc/ghc/-/work_items/27668); the body names it in prose and not by link, both reports having been written before either number existed, so the tracker copy is where a link between them can go.
...
A potential justification of GHC's behaviour is that GHC recompiles only when the old objects and the new objects would not work together, and that these settings make no such mismatch. That explanation does not hold. `-fspec-constr` makes no mismatch either: objects that are compiled with it and objects that are compiled without it link together and run correctly, thus that flag is only a question of speed. GHC does recompile for it, and it is the control in each test below. The settings in this report have the same property and GHC does not recompile for them. The one difference between the two groups is that `-fspec-constr` is a `GeneralFlag` value and they are not. Also, `-fignore-optim-changes` ("Do not recompile modules just to match changes to optimisation flags") gives a user the ability to stop the recompilation that a change of optimisation flags causes, and GHCi sets that option by default where a batch compilation does not. An option to stop a behaviour, and a mode that sets the option, are both necessary only when the behaviour is the default.

A second potential justification is that a user can give `-fforce-recomp`. That is a workaround and not a correction, for three reasons. A user finds it only when the user already knows about the defect, because the symptom is a build that is successful and gives no message; a user who does not know has no reason to look for it. `-fforce-recomp` turns recompilation checking off, thus a user who gives it always has removed the feature and not used it. A user of cabal pays for it by the package, because `--ghc-options` applies to each module of the package and the command line has no method to give the flag to only the modules whose settings changed. Under cabal the flag alone is also not sufficient: when it is the only change, cabal reports "Up to date" and does not start GHC, thus the flag has an effect only when some other part of the configuration changed also. And a user who changes `-fspec-constr` does not need it, thus a user must know which settings are in the fingerprint and which are not: the users guide gives no such list, and this report took it from the source of GHC.
...
The message is not only absent. It is incorrect: the module flags did change. The control gives a different message. With `-fspec-constr` in place of `-fllvm`, at `-O1`:
...
Four corrections are possible. (1) Add the absent fields to `IfaceDynFlags`. The record has this shape already: its field `ifaceCodeGenDistinctConstructorTables` holds `distinctConstructorTables`, of type `StgDebugDctConfig`, which is a setting that carries a value. `backend` and `cmmProcAlignment` need the same treatment. (2) Add the tool settings and the tool options. The same function has `ifaceCppSig = opt_P_signature dflags`, thus the options of the preprocessor are in the fingerprint. The options of the assembler, of `opt` and of `llc` are not, and the programs from the `-pgm*` flags are not. (3) Make the fingerprint an opt-out list and not an opt-in list. If the fingerprint uses all of `DynFlags` except the fields that are known to have no effect on the output, a new field is correct by default. Corrections (1) and (2) repair the known settings; correction (3) prevents the return of the defect. (4) A diagnosis aid, and not a correction: make `-ddump-hi-diffs` report which fields it compared. At present it says "Module flags unchanged" for a change of backend, and a user has no method to see which settings the comparison includes.

## Steps to reproduce

1. Save the program below as `Repro.hs`.
2. Compile the program and record the checksum: `ghc -O1 Repro.hs -o prog && md5sum prog`.
3. Compile the program again with one setting from the table above: `ghc -O1 -fllvm Repro.hs -o prog && md5sum prog`. GHC gives no message and the checksum does not change.
4. Compile the program again with the same setting and `-fforce-recomp`: `ghc -O1 -fllvm -fforce-recomp Repro.hs -o prog && md5sum prog`. The checksum changes. Thus the setting does change the generated code, and step 3 did not use it.
5. Repeat steps 2 to 4 for the other settings in the table. `-fllvm`, `-optlo` and `-optlc` need an installation of LLVM that GHC accepts; if you do not have one, put `-funfolding-use-threshold=1000` in place of `-fllvm` in steps 3 and 4, and the result is the same.

There is no table of results for each compiler, because the results do not differ: each of the four compilers in the Environment section gives the same behavior for each setting in the table. But read the recompilation from the message of GHC, and not from the checksum. GHC prints a `Compiling` line only when it recompiles. For the control `-fspec-constr` on 9.12.4, 9.14.1 and HEAD, GHC recompiles and makes an equal binary, because SpecConstr changes nothing in this module. A test that uses the checksum alone thus reports the control as a failure also. Use `-O1` for the control: at `-O2` SpecConstr is already active, and no recompilation is correct there.
...
There are three: give `-fforce-recomp`, or remove the `.o` and the `.hi` file of a module, or give a different `--builddir` for each configuration under cabal. All three are easy when you know the defect, and none of them is discoverable from the symptom, which is a build that is successful.

Under cabal each one has a limit. `-fforce-recomp` has an effect only when some other part of the configuration changed also, because cabal reports "Up to date" and does not start GHC when the flag is the only change. `-fforce-recomp` and a new build directory both compile each module of the package again, and not only the modules that the changed setting affects, because `--ghc-options` and a build directory apply to all of them; removal of the object files is the one that a user can point at a single module. For a measurement that goes from one configuration to the other and back, the build directory is the best of the three, because each configuration keeps its own objects and a return to a configuration compiles nothing.
