...
~1.36× loop-order factor of the Haskell copy path (the `gather48`
chains) and ~17% on a real shaped CNN gradient at 24×24 (the
`cnn-24x24` group); past that, term rewriting is exhausted and the
residual is the kernel itself. The staged upstream drafts therefore
...
the `IntMap` that a gather would not need. So `scatter48`'s ~0.55ms
against `gather48`'s ~4.9–6.7ms is a direct empirical bound: expect up to
~9–12× on the isolated gather chains, and — with interpreted gathers at
...
4. **Whether the branch's `shn`-sort remains a win over an `sv`-kernel
   ...
   already refuted by measurement — the sorted-scatter ~4.7×
