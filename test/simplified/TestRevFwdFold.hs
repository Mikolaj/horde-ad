...
testSin0Fold182SrevPP = do
  ...
  let a1 = rrev1 @(AstRanked FullSpan)
           (let f :: forall f. ADReadyS f => f Double '[] -> f Double '[5]
            ...
            in rfromS . f . sfromR) 1.1
  printAstPretty IM.empty a1
    @?= "rfromS (let m123 = sscanDer <lambda> <lambda> <lambda> (sreplicate (sfromR 1.1)) (sreplicate (sfromR 1.1)) in sfromR (let [v124 @[Natural] @Double @[5], v125 @[Natural] @Double @[1]] = dmapAccumRDer (SNat @1) <lambda> <lambda> <lambda> [sfromR (rreplicate 5 1.0)] [sslice m123, sreplicate (sfromR 1.1)] in rfromS (ssum v124) + rfromS (ssum v125)))"
...
testSin0Scan1RevPP1 = do
  ...
  printAstPrettyButNested IM.empty (simplifyAst6 a1)
    @?= "let v56 = rconst (fromList [2] [42.0,42.0]) in let [x67 @Natural @Double @[], v68 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) (\\[x69] [x70, x71, x72] -> [cos x71 * (x69 + x70), 0]) (\\[x73] [x74, x75, x76] [x77] [x78, x79, x80] -> [(x75 * negate (sin x79)) * (x77 + x78) + (x73 + x74) * cos x79, 0.0]) (\\[x86] [x87] [x88] [x89, x90, x91] -> let x96 = cos x90 * x86 in [x96, x96, negate (sin x90) * ((x88 + x89) * x86), 0]) [0.0] [rreplicate 2 1.0, rslice 0 2 (rscanDer (\\x57 x58 -> sin x57) (\\x59 x60 x61 x62 -> x59 * cos x61) (\\x63 x64 x65 -> [cos x64 * x63, 0]) 1.1 v56), v56] in x67 + 1.0"
...
testSin0ScanFwdPP = do
  ...
  let a1 = rfwd1 @(AstRanked FullSpan) @Double @0 @1
                 (\x0 -> rscan (\x _a -> sin x) x0
                           (rconst (OR.constant @Double @1 [2] 42))) 1.1
  printAstPrettyButNested IM.empty (simplifyAst6 a1)
    @?= "let v60 = rconst (fromList [2] [42.0,42.0]) in let [x71 @Natural @Double @[], v72 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) (\\[x73] [x74, x75, x76] -> let x77 = x73 * cos x75 in [x77, x77]) (\\[x78] [x79, x80, x81] [x82] [x83, x84, x85] -> let x89 = x78 * cos x84 + (x80 * negate (sin x84)) * x82 in [x89, x89]) (\\[x90] [x91] [x92] [x93, x94, x95] -> let x100 = x91 + x90 in [cos x94 * x100, 0, negate (sin x94) * (x92 * x100), 0]) [1.1] [rreplicate 2 0.0, rslice 0 2 (rscanDer (\\x61 x62 -> sin x61) (\\x63 x64 x65 x66 -> x63 * cos x65) (\\x67 x68 x69 -> [cos x68 * x67, 0]) 1.1 v60), v60] in rappend (rreplicate 1 1.1) v72"
...
testSin0Scan1Rev2PP1 = do
  ...
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v63 = rconst (fromList [2] [5.0,7.0]) in let [x75 @Natural @Double @[], v76 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) <lambda> <lambda> <lambda> [0.0] [rreplicate 2 1.0, rslice 0 2 (rscanDer <lambda> <lambda> <lambda> 1.1 v63), v63] in x75 + 1.0"
...
testSin0Scan1Rev2PPA = do
  ...
  printAstHVectorPretty IM.empty (simplifyAstHVector6 a1)
    @?= "let v15 = rconst (fromList [2] [5.0,7.0]) in let [x60 @Natural @Double @[], v61 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) <lambda> <lambda> <lambda> [0.0] [rslice 1 2 v26, rslice 0 2 (rscanDer <lambda> <lambda> <lambda> x1 v15), v15] in [x60 + v26 ! [0]]"
...
testSin0Scan1Rev3PP = do
  ...
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v63 = rfromList [1.1 * 5.0, 1.1 * 7.0] in let [x75 @Natural @Double @[], v76 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) <lambda> <lambda> <lambda> [0.0] [rreplicate 2 1.0, rslice 0 2 (rscanDer <lambda> <lambda> <lambda> 1.1 v63), v63] in 5.0 * v76 ! [0] + 7.0 * v76 ! [1] + x75 + 1.0"
...
testSin0ScanFwd3PP = do
  ...
  let a1 = rfwd1 @(AstRanked FullSpan) @Double @0 @1
                 (\x0 -> rscan (\x a -> sin x - a) x0
                           (rfromList [x0 * 5, x0 * 7])) 1.1
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v72 = rfromList [1.1 * 5.0, 1.1 * 7.0] in let [x87 @Natural @Double @[], v88 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) <lambda> <lambda> <lambda> [1.1] [rfromList [1.1 * 5.0, 1.1 * 7.0], rslice 0 2 (rscanDer <lambda> <lambda> <lambda> 1.1 v72), v72] in rappend (rreplicate 1 1.1) v88"
...
testSin0rmapAccumRD01SN531b0PP = do
  ...
  printAstHVectorPrettyButNested
    ...
    @?= "let v58 = rconst (fromList [0] []) in let [x59 @[Natural] @Double @[], v60 @[Natural] @Double @[0]] = dmapAccumRDer (SNat @0) (\\[x61] [x62] -> [x61, x61]) (\\[x63] [x64] [x65] [x66] -> [x63, x63]) (\\[x67] [x68] [x69] [x70] -> [0.0 + x67 + x68, 0]) [sfromR 1.1] [v58] in let [x74 @[Natural] @Double @[], v75 @Natural @Double @[0]] = dmapAccumLDer (SNat @0) (\\[x76] [x77, x78] -> [x76, 0]) (\\[x79] [x80, x81] [x82] [x83, x84] -> [x79, 0.0]) (\\[x85] [x86] [x87] [x88, x89] -> [x85, 0, 0]) [sfromR (rsum (rreplicate 4 1.0))] [v60, v58] in [rfromS x74]"
...
testSin0rmapAccumRD01SN531bSPP = do
  ...
  printAstHVectorPretty
    ...
    @?= "let v58 = sconst @[1] (fromList @[1] [0.0]) in let [x59 @[Natural] @Double @[], v60 @[Natural] @Double @[1]] = dmapAccumRDer (SNat @1) <lambda> <lambda> <lambda> [1.1] [v58] in let [x74 @[Natural] @Double @[], v75 @[Natural] @Double @[1]] = dmapAccumLDer (SNat @1) <lambda> <lambda> <lambda> [sfromR (rsum (rgather [4] (rfromS (sconst @[2,2] (fromList @[2,2] [1.0,1.0,1.0,1.0]))) (\\[i92] -> [rem (quot i92 2) 2, rem i92 2])))] [v60, v58] in [x74]"
...
testSin0rmapAccumRD01SN531bRPP = do
  ...
  printAstHVectorSimple
    ...
    (simplifyAstHVector6
     $ g @(AstRanked FullSpan) (V.singleton $ DynamicRanked @Double @0 1.1))
    @?= "rletInHVector (rconstant (rconst (fromList [1] [0.0]))) (\\v58 -> dletHVectorInHVector (dmapAccumRDer (SNat @1) (\\[x61 @Natural @Double @[]] [x62 @Natural @Double @[]] -> dmkHVector (fromList [DynamicRanked x61, DynamicRanked x61])) (\\[x63 @Natural @Double @[]] [x64 @Natural @Double @[]] [x65 @Natural @Double @[]] [x66 @Natural @Double @[]] -> dmkHVector (fromList [DynamicRanked x63, DynamicRanked x63])) (\\[x67 @Natural @Double @[]] [x68 @Natural @Double @[]] [x69 @Natural @Double @[]] [x70 @Natural @Double @[]] -> dmkHVector (fromList [DynamicRanked (x68 + x67), DynamicRankedDummy])) (fromList [DynamicRanked (rconstant 1.1)]) (fromList [DynamicRanked v58])) (\\[x59 @Natural @Double @[], v60 @Natural @Double @[1]] -> dletHVectorInHVector (dmapAccumLDer (SNat @1) (\\[x76 @Natural @Double @[]] [x77 @Natural @Double @[], x78 @Natural @Double @[]] -> dmkHVector (fromList [DynamicRanked x76, DynamicRankedDummy])) (\\[x79 @Natural @Double @[]] [x80 @Natural @Double @[], x81 @Natural @Double @[]] [x82 @Natural @Double @[]] [x83 @Natural @Double @[], x84 @Natural @Double @[]] -> dmkHVector (fromList [DynamicRanked x79, DynamicRanked 0.0])) (\\[x85 @Natural @Double @[]] [x86 @Natural @Double @[]] [x87 @Natural @Double @[]] [x88 @Natural @Double @[], x89 @Natural @Double @[]] -> dmkHVector (fromList [DynamicRanked x85, DynamicRankedDummy, DynamicRankedDummy])) (fromList [DynamicRanked (rconstant (rsum (rreplicate 4 1.0)))]) (fromList [DynamicRanked v60, DynamicRanked v58])) (\\[x74 @Natural @Double @[], v75 @Natural @Double @[1]] -> dmkHVector (fromList [DynamicRanked x74]))))"
...
testSin0rmapAccumRD01SN531b0PPj = do
  ...
  printAstHVectorPretty
    ...
    (simplifyAstHVector6
     $ g @(AstRanked FullSpan) (V.singleton $ DynamicRanked @Double @0 1.1))
    @?= "let t143 = rtranspose [2,0,1] (rreplicate 2 (rreplicate 2 (rconst (fromList [0] [])))) in let [m144 @[Natural] @Double @[2,2], t145 @[Natural] @Double @[0,2,2]] = dmapAccumRDer (SNat @0) <lambda> <lambda> <lambda> [sfromIntegral (sfromR (rtranspose [1,0] (rreplicate 2 (rconst (fromList [2] [0,1]))) + rreplicate 2 (rconst (fromList [2] [0,1])))) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0]) + sreplicate (sconst @[2] (fromList @[2] [0.0,0.0])) + sreplicate (sreplicate (sfromR 1.1)) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0]) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0]) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0])] [t143] in let [m180 @[Natural] @Double @[2,2], t181 @Natural @Double @[0,2,2]] = dmapAccumLDer (SNat @0) <lambda> <lambda> <lambda> [sscatter (sfromR (rreplicate 2 (rreplicate 2 1.0))) (\\[i179] -> [i179])] [t145, t143] in [rfromS (ssum (ssum m180))]"
...
testSin0rmapAccumRD01SN531bSPPj = do
  ...
  printAstHVectorPretty
    ...
    (simplifyAstHVector6
     $ g @(AstRanked FullSpan) (V.singleton $ DynamicShaped @Double @'[] 1.1))
    @?= "let t144 = stranspose (sreplicate (sreplicate (sconst @[1] (fromList @[1] [0.0])))) in let [m145 @[Natural] @Double @[2,2], t146 @[Natural] @Double @[1,2,2]] = dmapAccumRDer (SNat @1) <lambda> <lambda> <lambda> [sfromIntegral (sfromR (rtranspose [1,0] (rreplicate 2 (rconst (fromList [2] [0,1]))) + rreplicate 2 (rconst (fromList [2] [0,1])))) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0]) + sreplicate (sconst @[2] (fromList @[2] [0.0,0.0])) + sreplicate (sreplicate 1.1) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0]) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0]) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0])] [t144] in let [m181 @[Natural] @Double @[2,2], t182 @[Natural] @Double @[1,2,2]] = dmapAccumLDer (SNat @1) <lambda> <lambda> <lambda> [sscatter (sconst @[2,2] (fromList @[2,2] [1.0,1.0,1.0,1.0])) (\\[i180] -> [i180])] [t146, t144] in [ssum (ssum m181)]"
...
testSin0rmapAccumRD01SN531bRPPj = do
  ...
  printAstHVectorPretty
    ...
    (simplifyAstHVector6
     $ g @(AstRanked FullSpan) (V.singleton $ DynamicRanked @Double @0 1.1))
    @?= "let t61 = rtranspose [2,0,1] (rreplicate 2 (rreplicate 2 (rconst (fromList [1] [0.0])))) in let [m62 @Natural @Double @[2,2], t63 @Natural @Double @[1,2,2]] = dmapAccumRDer (SNat @1) <lambda> <lambda> <lambda> [rfromIntegral (rtranspose [1,0] (rreplicate 2 (rconst (fromList [2] [0,1]))) + rreplicate 2 (rconst (fromList [2] [0,1]))) + rreplicate 2 (rreplicate 2 1.1)] [t61] in let [m77 @Natural @Double @[2,2], t78 @Natural @Double @[1,2,2]] = dmapAccumLDer (SNat @1) <lambda> <lambda> <lambda> [rreplicate 2 (rreplicate 2 1.0)] [t63, t61] in [rsum (rreshape [4] m77)]"
  ...
  let a1 = rrev1 @(AstRanked FullSpan) @Double @0 @1
                 (\x0 -> rscanZip (\x _a -> sin x)
                           ...
                           x0 (V.singleton $ DynamicRanked
                               $ rconst (OR.constant @Double @1 [2] 42))) 1.1
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v97 = rconst (fromList [2] [42.0,42.0]) in let [x98 @Natural @Double @[], v99 @Natural @Double @[2], v100 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) <lambda> <lambda> <lambda> [1.1] [v97] in let [x121 @Natural @Double @[], v122 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) <lambda> <lambda> <lambda> [rfromS 0.0] [rreplicate 2 1.0, v99, v97] in x121 + 1.0"
  ...
  let a1 = rfwd1 @(AstRanked FullSpan) @Double @0 @1
                 (\x0 -> rscanZip (\x _a -> sin x)
                           ...
                           x0 (V.singleton $ DynamicRanked
                               $ rconst (OR.constant @Double @1 [2] 42))) 1.1
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v94 = rconst (fromList [2] [42.0,42.0]) in let [x95 @Natural @Double @[], v96 @Natural @Double @[2], v97 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) <lambda> <lambda> <lambda> [1.1] [v94] in let [x99 @Natural @Double @[], v100 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) <lambda> <lambda> <lambda> [1.1] [rreplicate 2 0.0, v96, v94] in rappend (rreplicate 1 1.1) v100"
  ...
  let a1 = rrev1 @(AstRanked FullSpan) @Double @0 @1
                 (\x0 -> rscanZip (\x a -> sin x - rfromD (a V.! 0))
                         ...
                         x0 (V.singleton $ DynamicRanked
                             $ rconst (OR.fromList @Double @1 [2] [5, 7]))) 1.1
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v105 = rconst (fromList [2] [5.0,7.0]) in let [x106 @Natural @Double @[], v107 @Natural @Double @[2], v108 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) <lambda> <lambda> <lambda> [1.1] [v105] in let [x131 @Natural @Double @[], v132 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) <lambda> <lambda> <lambda> [rfromS 0.0] [rreplicate 2 1.0, v107, v105] in x131 + 1.0"
...
testSin0ScanDFwd2PP = do
  ...
  let a1 = rfwd1 @(AstRanked FullSpan) @Double @0 @1
                 (\x0 -> rscanZip (\x a -> sin x - rfromD (a V.! 0))
                         ...
                         $ rconst (OR.fromList @Double @1 [2] [5, 7]))) 1.1
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v105 = rconst (fromList [2] [5.0,7.0]) in let [x106 @Natural @Double @[], v107 @Natural @Double @[2], v108 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) <lambda> <lambda> <lambda> [1.1] [v105] in let [x110 @Natural @Double @[], v111 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) <lambda> <lambda> <lambda> [1.1] [rreplicate 2 0.0, v107, v105] in rappend (rreplicate 1 1.1) v111"
  ...
  let a1 = rfwd1 @(AstRanked FullSpan) @Double @0 @1
                 (\x0 -> rscanZip (\x a -> sin x - rfromD (a V.! 0))
                                ...
                                x0 (V.singleton $ DynamicRanked
                                    $ rfromList [x0 * 5, x0 * 7])) 1.1
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v108 = rfromList [1.1 * 5.0, 1.1 * 7.0] in let [x109 @Natural @Double @[], v110 @Natural @Double @[2], v111 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) <lambda> <lambda> <lambda> [1.1] [v108] in let [x116 @Natural @Double @[], v117 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) <lambda> <lambda> <lambda> [1.1] [rfromList [1.1 * 5.0, 1.1 * 7.0], v110, v108] in rappend (rreplicate 1 1.1) v117"
...
testSin0FoldNestedS1PP = do
  ...
  printAstHVectorPretty
    IM.empty
    (g @(AstRanked FullSpan) (V.singleton $ DynamicShaped @Double @'[] 1.1))
    @?= "let v920 = sscanDer <lambda> <lambda> <lambda> 1.1 (sreplicate 1.1) in let [x921 @[Natural] @Double @[], v922 @[Natural] @Double @[11]] = dmapAccumRDer (SNat @11) <lambda> <lambda> <lambda> [1.0] [sslice v920, sreplicate 1.1] in [x921 + 0.0 + ssum v922]"
...
testSin0FoldNestedR1PP = do
  ...
  printAstHVectorPretty
    IM.empty
    (g @(AstRanked FullSpan) (V.singleton $ DynamicRanked @Double @0 1.1))
    @?= "let v921 = rscanDer <lambda> <lambda> <lambda> 1.1 (rreplicate 11 1.1) ; x922 = rreshape [] (rreplicate 1 1.0) in let [x923 @Natural @Double @[], v924 @Natural @Double @[11]] = dmapAccumRDer (SNat @11) <lambda> <lambda> <lambda> [x922] [rslice 0 11 v921, rreplicate 11 1.1] in [rsum v924 + x923]"
...
testSin0FoldNestedR1SimpPP = do
  ...
  printAstHVectorPretty
    ...
    (simplifyAstHVector6
     $ g @(AstRanked FullSpan) (V.singleton $ DynamicRanked @Double @0 1.1))
    @?= "let [x923 @Natural @Double @[], v924 @Natural @Double @[11]] = dmapAccumRDer (SNat @11) <lambda> <lambda> <lambda> [1.0] [rslice 0 11 (rscanDer <lambda> <lambda> <lambda> 1.1 (rreplicate 11 1.1)), rreplicate 11 1.1] in [rsum v924 + x923]"
...
testSin0FoldNestedR1SimpNestedPP = do
  ...
  printAstHVectorPrettyButNested
    ...
    (simplifyAstHVector6
     $ g @(AstRanked FullSpan) (V.singleton $ DynamicRanked @Double @0 1.1))
    @?= "let [x923 @Natural @Double @[], v924 @Natural @Double @[11]] = dmapAccumRDer (SNat @11) (\\[x925] [x926, x927] -> let [x941 @Natural @Double @[], v942 @Natural @Double @[22]] = dmapAccumRDer (SNat @22) (\\[x943] [x944, x945] -> let x946 = cos x945 in [x943, recip (x946 * x946) * x943]) (\\[x947] [x948, x949] [x950] [x951, x952] -> let x953 = cos x952 ; x954 = x953 * x953 ; x956 = x949 * negate (sin x952) in [x947, ((x956 * x953 + x956 * x953) * negate (recip (x954 * x954))) * x950 + x947 * recip x954]) (\\[x960] [x961] [x962] [x963, x964] -> let x965 = cos x964 ; x966 = x965 * x965 ; x968 = negate (recip (x966 * x966)) * (x962 * x961) in [recip x966 * x961 + x960, 0, negate (sin x964) * (x965 * x968 + x965 * x968)]) [x925] [rslice 0 22 (rscanDer (\\x928 x929 -> x928 + tan x929) (\\x930 x931 x932 x933 -> let x934 = cos x933 in x930 + x931 * recip (x934 * x934)) (\\x936 x937 x938 -> let x939 = cos x938 in [x936, recip (x939 * x939) * x936]) x927 (rreplicate 22 x926)), rreplicate 22 x926] in [rsum v942, x941]) (\\[x969] [x970, x971] [x972] [x973, x974] -> let v987 = rscanDer (\\x975 x976 -> x975 + tan x976) (\\x977 x978 x979 x980 -> let x981 = cos x980 in x977 + x978 * recip (x981 * x981)) (\\x983 x984 x985 -> let x986 = cos x985 in [x983, recip (x986 * x986) * x983]) x974 (rreplicate 22 x973) in let [x990 @Natural @Double @[], v991 @Natural @Double @[22], v992 @Natural @Double @[22]] = dmapAccumRDer (SNat @22) (\\[x993] [x994, x995] -> let x996 = cos x995 in [x993, x993, recip (x996 * x996) * x993]) (\\[x998] [x999, x1000] [x1001] [x1002, x1003] -> let x1004 = cos x1003 ; x1005 = x1004 * x1004 ; x1008 = x1000 * negate (sin x1003) in [x998, x998, ((x1008 * x1004 + x1008 * x1004) * negate (recip (x1005 * x1005))) * x1001 + x998 * recip x1005]) (\\[x1012] [x1013, x1014] [x1015] [x1016, x1017] -> let x1018 = cos x1017 ; x1019 = x1018 * x1018 ; x1025 = negate (recip (x1019 * x1019)) * (x1015 * x1014) in [recip x1019 * x1014 + x1013 + x1012, 0, negate (sin x1017) * (x1018 * x1025 + x1018 * x1025)]) [x972] [rslice 0 22 v987, rreplicate 22 x973] in let [x1069 @Natural @Double @[], v1070 @Natural @Double @[22]] = dmapAccumRDer (SNat @22) (\\[x1071] [x1072, x1073, x1074, x1075, x1076] -> let x1077 = cos x1076 ; x1078 = x1077 * x1077 ; x1080 = x1073 * negate (sin x1076) in [x1071, ((x1080 * x1077 + x1080 * x1077) * negate (recip (x1078 * x1078))) * x1074 + x1071 * recip x1078]) (\\[x1084] [x1085, x1086, x1087, x1088, x1089] [x1090] [x1091, x1092, x1093, x1094, x1095] -> let x1096 = cos x1095 ; x1097 = x1096 * x1096 ; x1099 = negate (sin x1095) ; x1100 = x1092 * x1099 ; x1101 = x1100 * x1096 + x1100 * x1096 ; x1102 = x1097 * x1097 ; x1103 = negate (recip x1102) ; x1108 = x1086 * x1099 + ((x1089 * cos x1095) * -1.0) * x1092 ; x1109 = x1089 * negate (sin x1095) ; x1113 = x1109 * x1096 + x1109 * x1096 in [x1084, ((x1108 * x1096 + x1109 * x1100 + x1108 * x1096 + x1109 * x1100) * x1103 + (((x1113 * x1097 + x1113 * x1097) * negate (recip (x1102 * x1102))) * -1.0) * x1101) * x1093 + x1087 * (x1101 * x1103) + x1084 * recip x1097 + (x1113 * negate (recip (x1097 * x1097))) * x1090]) (\\[x1122] [x1123] [x1124] [x1125, x1126, x1127, x1128, x1129] -> let x1130 = cos x1129 ; x1131 = x1130 * x1130 ; x1133 = negate (sin x1129) ; x1134 = x1126 * x1133 ; x1135 = x1134 * x1130 + x1134 * x1130 ; x1136 = x1131 * x1131 ; x1137 = negate (recip x1136) ; x1142 = x1127 * x1123 ; x1143 = negate (recip (x1136 * x1136)) * (-1.0 * (x1135 * x1142)) ; x1144 = x1137 * x1142 ; x1145 = x1130 * x1144 + x1130 * x1144 ; x1146 = negate (recip (x1131 * x1131)) * (x1124 * x1123) + x1131 * x1143 + x1131 * x1143 in [recip x1131 * x1123 + x1122, 0, x1133 * x1145, (x1135 * x1137) * x1123, 0, negate (sin x1129) * (x1130 * x1146 + x1130 * x1146 + x1134 * x1144 + x1134 * x1144) + cos x1129 * (-1.0 * (x1126 * x1145))]) [x969] [rslice 0 22 (let [x1026 @Natural @Double @[], v1027 @Natural @Double @[22]] = dmapAccumLDer (SNat @22) (\\[x1028] [x1029, x1030, x1031] -> let x1034 = let x1032 = cos x1031 in x1028 + x1029 * recip (x1032 * x1032) in [x1034, x1034]) (\\[x1035] [x1036, x1037, x1038] [x1039] [x1040, x1041, x1042] -> let x1043 = cos x1042 ; x1044 = x1043 * x1043 ; x1048 = x1038 * negate (sin x1042) ; x1052 = x1035 + x1036 * recip x1044 + ((x1048 * x1043 + x1048 * x1043) * negate (recip (x1044 * x1044))) * x1040 in [x1052, x1052]) (\\[x1053] [x1054] [x1055] [x1056, x1057, x1058] -> let x1059 = cos x1058 ; x1060 = x1059 * x1059 ; x1066 = x1054 + x1053 ; x1067 = negate (recip (x1060 * x1060)) * (x1056 * x1066) in [x1066, recip x1060 * x1066, 0, negate (sin x1058) * (x1059 * x1067 + x1059 * x1067)]) [x971] [rreplicate 22 x970, rslice 0 22 v987, rreplicate 22 x973] in rappend (rreplicate 1 x971) v1027), rreplicate 22 x970, v991, rslice 0 22 v987, rreplicate 22 x973] in [rsum v1070, x1069]) (\\[x1148] [x1149] [x1150] [x1151, x1152] -> let v1165 = rscanDer (\\x1153 x1154 -> x1153 + tan x1154) (\\x1155 x1156 x1157 x1158 -> let x1159 = cos x1158 in x1155 + x1156 * recip (x1159 * x1159)) (\\x1161 x1162 x1163 -> let x1164 = cos x1163 in [x1161, recip (x1164 * x1164) * x1161]) x1152 (rreplicate 22 x1151) in let [x1168 @Natural @Double @[], v1169 @Natural @Double @[22], v1170 @Natural @Double @[22]] = dmapAccumRDer (SNat @22) (\\[x1171] [x1172, x1173] -> let x1174 = cos x1173 in [x1171, x1171, recip (x1174 * x1174) * x1171]) (\\[x1176] [x1177, x1178] [x1179] [x1180, x1181] -> let x1182 = cos x1181 ; x1183 = x1182 * x1182 ; x1186 = x1178 * negate (sin x1181) in [x1176, x1176, ((x1186 * x1182 + x1186 * x1182) * negate (recip (x1183 * x1183))) * x1179 + x1176 * recip x1183]) (\\[x1190] [x1191, x1192] [x1193] [x1194, x1195] -> let x1196 = cos x1195 ; x1197 = x1196 * x1196 ; x1203 = negate (recip (x1197 * x1197)) * (x1193 * x1192) in [recip x1197 * x1192 + x1191 + x1190, 0, negate (sin x1195) * (x1196 * x1203 + x1196 * x1203)]) [x1150] [rslice 0 22 v1165, rreplicate 22 x1151] in let [x1208 @Natural @Double @[], v1209 @Natural @Double @[22], v1210 @Natural @Double @[22]] = dmapAccumLDer (SNat @22) (\\[x1211] [x1212, x1213, x1214, x1215] -> let x1216 = cos x1215 ; x1217 = x1216 * x1216 ; x1219 = negate (recip (x1217 * x1217)) * (x1213 * x1212) in [recip x1217 * x1212 + x1211, 0, negate (sin x1215) * (x1216 * x1219 + x1216 * x1219)]) (\\[x1220] [x1221, x1222, x1223, x1224] [x1225] [x1226, x1227, x1228, x1229] -> let x1230 = cos x1229 ; x1231 = x1230 * x1230 ; x1233 = x1231 * x1231 ; x1234 = negate (recip x1233) ; x1235 = x1227 * x1226 ; x1236 = x1234 * x1235 ; x1239 = x1224 * negate (sin x1229) ; x1240 = x1239 * x1230 + x1239 * x1230 ; x1250 = (((x1240 * x1231 + x1240 * x1231) * negate (recip (x1233 * x1233))) * -1.0) * x1235 + (x1222 * x1226 + x1221 * x1227) * x1234 in [x1220 + (x1240 * negate (recip (x1231 * x1231))) * x1226 + x1221 * recip x1231, 0.0, ((x1224 * cos x1229) * -1.0) * (x1230 * x1236 + x1230 * x1236) + (x1239 * x1236 + x1250 * x1230 + x1239 * x1236 + x1250 * x1230) * negate (sin x1229)]) (\\[x1255] [x1256, x1257] [x1258] [x1259, x1260, x1261, x1262] -> let x1263 = cos x1262 ; x1264 = x1263 * x1263 ; x1266 = x1264 * x1264 ; x1267 = negate (recip x1266) ; x1268 = x1260 * x1259 ; x1269 = x1267 * x1268 ; x1275 = negate (sin x1262) * x1257 ; x1276 = x1263 * x1275 + x1263 * x1275 ; x1277 = x1267 * x1276 ; x1278 = negate (recip (x1266 * x1266)) * (-1.0 * (x1268 * x1276)) ; x1279 = negate (recip (x1264 * x1264)) * (x1259 * x1255) + x1264 * x1278 + x1264 * x1278 in [x1255, x1260 * x1277 + recip x1264 * x1255, x1259 * x1277, 0, negate (sin x1262) * (x1263 * x1279 + x1263 * x1279 + x1269 * x1275 + x1269 * x1275) + cos x1262 * (-1.0 * ((x1263 * x1269 + x1263 * x1269) * x1257))]) [x1149] [rreplicate 22 x1148, v1169, rslice 0 22 v1165, rreplicate 22 x1151] in let v1280 = rappend (rreplicate 0 0.0) (rappend v1209 (rreplicate 1 0.0)) in let [x1281 @Natural @Double @[], v1282 @Natural @Double @[22]] = dmapAccumRDer (SNat @22) (\\[x1283] [x1284, x1285, x1286] -> let x1287 = cos x1286 in [x1283 + x1284, recip (x1287 * x1287) * (x1283 + x1284)]) (\\[x1288] [x1289, x1290, x1291] [x1292] [x1293, x1294, x1295] -> let x1296 = cos x1295 ; x1297 = x1296 * x1296 ; x1301 = x1291 * negate (sin x1295) in [x1288 + x1289, ((x1301 * x1296 + x1301 * x1296) * negate (recip (x1297 * x1297))) * (x1292 + x1293) + (x1288 + x1289) * recip x1297]) (\\[x1306] [x1307] [x1308] [x1309, x1310, x1311] -> let x1312 = cos x1311 ; x1313 = x1312 * x1312 ; x1318 = recip x1313 * x1307 ; x1319 = negate (recip (x1313 * x1313)) * ((x1308 + x1309) * x1307) in [x1306 + x1318, x1306 + x1318, 0, negate (sin x1311) * (x1312 * x1319 + x1312 * x1319)]) [0.0] [rslice 1 22 v1280, rslice 0 22 v1165, rreplicate 22 x1151] in [x1208, rsum v1282 + rsum v1210, x1281 + v1280 ! [0]]) [1.0] [rslice 0 11 (rscanDer (\\x809 x810 -> rfoldDer (\\x811 x812 -> x811 + tan x812) (\\x813 x814 x815 x816 -> let x817 = cos x816 in x813 + x814 * recip (x817 * x817)) (\\x819 x820 x821 -> let x822 = cos x821 in [x819, recip (x822 * x822) * x819]) x810 (rreplicate 22 x809)) (\\x823 x824 x825 x826 -> let [x840 @Natural @Double @[]] = dmapAccumLDer (SNat @22) (\\[x841] [x842, x843, x844] -> [let x845 = cos x844 in x841 + x842 * recip (x845 * x845)]) (\\[x848] [x849, x850, x851] [x852] [x853, x854, x855] -> let x856 = cos x855 ; x857 = x856 * x856 ; x861 = x851 * negate (sin x855) in [x848 + x849 * recip x857 + ((x861 * x856 + x861 * x856) * negate (recip (x857 * x857))) * x853]) (\\[x866] [] [x867] [x868, x869, x870] -> let x871 = cos x870 ; x872 = x871 * x871 ; x876 = negate (recip (x872 * x872)) * (x868 * x866) in [x866, recip x872 * x866, 0, negate (sin x870) * (x871 * x876 + x871 * x876)]) [x824] [rreplicate 22 x823, rslice 0 22 (rscanDer (\\x827 x828 -> x827 + tan x828) (\\x829 x830 x831 x832 -> let x833 = cos x832 in x829 + x830 * recip (x833 * x833)) (\\x835 x836 x837 -> let x838 = cos x837 in [x835, recip (x838 * x838) * x835]) x826 (rreplicate 22 x825)), rreplicate 22 x825] in x840) (\\x877 x878 x879 -> let [x893 @Natural @Double @[], v894 @Natural @Double @[22]] = dmapAccumRDer (SNat @22) (\\[x895] [x896, x897] -> let x898 = cos x897 in [x895, recip (x898 * x898) * x895]) (\\[x899] [x900, x901] [x902] [x903, x904] -> let x905 = cos x904 ; x906 = x905 * x905 ; x908 = x901 * negate (sin x904) in [x899, ((x908 * x905 + x908 * x905) * negate (recip (x906 * x906))) * x902 + x899 * recip x906]) (\\[x912] [x913] [x914] [x915, x916] -> let x917 = cos x916 ; x918 = x917 * x917 ; x920 = negate (recip (x918 * x918)) * (x914 * x913) in [recip x918 * x913 + x912, 0, negate (sin x916) * (x917 * x920 + x917 * x920)]) [x877] [rslice 0 22 (rscanDer (\\x880 x881 -> x880 + tan x881) (\\x882 x883 x884 x885 -> let x886 = cos x885 in x882 + x883 * recip (x886 * x886)) (\\x888 x889 x890 -> let x891 = cos x890 in [x888, recip (x891 * x891) * x888]) x879 (rreplicate 22 x878)), rreplicate 22 x878] in [rsum v894, x893]) 1.1 (rreplicate 11 1.1)), rreplicate 11 1.1] in [rsum v924 + x923]"
...
testSin0MapAccumNestedR1PP = do
  ...
  printAstHVectorPrettyButNested
    ...
    (simplifyAstHVector6
     $ g @(AstRanked FullSpan) (V.singleton $ DynamicRanked @Double @0 1.1))
    @?= "let [x990 @Natural @Double @[], v991 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) (\\[x992] [x993] -> let [x994 @Natural @Double @[]] = dmapAccumLDer (SNat @2) (\\[x995] [x996] -> [x995 + tan x996]) (\\[x997] [x998] [x999] [x1000] -> let x1001 = cos x1000 in [x997 + x998 * recip (x1001 * x1001)]) (\\[x1004] [] [x1005] [x1006] -> let x1007 = cos x1006 in [x1004, recip (x1007 * x1007) * x1004]) [x993] [rreplicate 2 x992] in [x994, x992]) (\\[x1008] [x1009] [x1010] [x1011] -> let [x1013 @Natural @Double @[], v1014 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) (\\[x1015] [x1016] -> [x1015 + tan x1016, x1015]) (\\[x1018] [x1019] [x1020] [x1021] -> let x1022 = cos x1021 in [x1018 + x1019 * recip (x1022 * x1022), x1018]) (\\[x1026] [x1027] [x1028] [x1029] -> let x1030 = cos x1029 in [x1026 + x1027, recip (x1030 * x1030) * x1026]) [x1011] [rreplicate 2 x1010] in let [x1034 @Natural @Double @[]] = dmapAccumLDer (SNat @2) (\\[x1035] [x1036, x1037, x1038] -> let x1039 = cos x1038 in [x1035 + x1036 * recip (x1039 * x1039)]) (\\[x1042] [x1043, x1044, x1045] [x1046] [x1047, x1048, x1049] -> let x1050 = cos x1049 ; x1051 = x1050 * x1050 ; x1055 = x1045 * negate (sin x1049) in [x1042 + x1043 * recip x1051 + ((x1055 * x1050 + x1055 * x1050) * negate (recip (x1051 * x1051))) * x1047]) (\\[x1060] [] [x1061] [x1062, x1063, x1064] -> let x1065 = cos x1064 ; x1066 = x1065 * x1065 ; x1071 = negate (recip (x1066 * x1066)) * (x1062 * x1060) in [x1060, recip x1066 * x1060, 0, negate (sin x1064) * (x1065 * x1071 + x1065 * x1071)]) [x1009] [rreplicate 2 x1008, v1014, rreplicate 2 x1010] in [x1034, x1008]) (\\[x1072] [x1073] [x1074] [x1075] -> let [x1077 @Natural @Double @[], v1078 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) (\\[x1079] [x1080] -> [x1079 + tan x1080, x1079]) (\\[x1082] [x1083] [x1084] [x1085] -> let x1086 = cos x1085 in [x1082 + x1083 * recip (x1086 * x1086), x1082]) (\\[x1090] [x1091] [x1092] [x1093] -> let x1094 = cos x1093 in [x1090 + x1091, recip (x1094 * x1094) * x1090]) [x1075] [rreplicate 2 x1074] in let [x1101 @Natural @Double @[], v1102 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) (\\[x1103] [x1104, x1105] -> let x1106 = cos x1105 in [x1103, recip (x1106 * x1106) * x1103]) (\\[x1107] [x1108, x1109] [x1110] [x1111, x1112] -> let x1113 = cos x1112 ; x1114 = x1113 * x1113 ; x1116 = x1109 * negate (sin x1112) in [x1107, ((x1116 * x1113 + x1116 * x1113) * negate (recip (x1114 * x1114))) * x1110 + x1107 * recip x1114]) (\\[x1120] [x1121] [x1122] [x1123, x1124] -> let x1125 = cos x1124 ; x1126 = x1125 * x1125 ; x1130 = negate (recip (x1126 * x1126)) * (x1122 * x1121) in [recip x1126 * x1121 + x1120, 0, negate (sin x1124) * (x1125 * x1130 + x1125 * x1130)]) [x1072] [v1078, rreplicate 2 x1074] in [rsum v1102 + x1073, x1101]) [1.1] [rreplicate 2 1.1] in let [x1132 @Natural @Double @[], v1133 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) (\\[x1134] [x1135, x1136] -> let [x1137 @Natural @Double @[], v1138 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) (\\[x1139] [x1140] -> [x1139 + tan x1140, x1139]) (\\[x1142] [x1143] [x1144] [x1145] -> let x1146 = cos x1145 in [x1142 + x1143 * recip (x1146 * x1146), x1142]) (\\[x1150] [x1151] [x1152] [x1153] -> let x1154 = cos x1153 in [x1150 + x1151, recip (x1154 * x1154) * x1150]) [x1136] [rreplicate 2 x1135] in let [x1156 @Natural @Double @[], v1157 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) (\\[x1158] [x1159, x1160] -> let x1161 = cos x1160 in [x1158, recip (x1161 * x1161) * x1158]) (\\[x1162] [x1163, x1164] [x1165] [x1166, x1167] -> let x1168 = cos x1167 ; x1169 = x1168 * x1168 ; x1171 = x1164 * negate (sin x1167) in [x1162, ((x1171 * x1168 + x1171 * x1168) * negate (recip (x1169 * x1169))) * x1165 + x1162 * recip x1169]) (\\[x1175] [x1176] [x1177] [x1178, x1179] -> let x1180 = cos x1179 ; x1181 = x1180 * x1180 ; x1183 = negate (recip (x1181 * x1181)) * (x1177 * x1176) in [recip x1181 * x1176 + x1175, 0, negate (sin x1179) * (x1180 * x1183 + x1180 * x1183)]) [x1134] [v1138, rreplicate 2 x1135] in [rsum v1157, x1156]) (\\[x1184] [x1185, x1186] [x1187] [x1188, x1189] -> let [x1191 @Natural @Double @[], v1192 @Natural @Double @[2], v1193 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) (\\[x1194] [x1195] -> [x1194 + tan x1195, x1194, x1194]) (\\[x1197] [x1198] [x1199] [x1200] -> let x1201 = cos x1200 in [x1197 + x1198 * recip (x1201 * x1201), x1197, x1197]) (\\[x1205] [x1206, x1207] [x1208] [x1209] -> let x1210 = cos x1209 in [x1205 + x1207 + x1206, recip (x1210 * x1210) * x1205]) [x1189] [rreplicate 2 x1188] in let [x1217 @Natural @Double @[], v1218 @Natural @Double @[2], v1219 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) (\\[x1220] [x1221, x1222] -> let x1223 = cos x1222 in [x1220, x1220, recip (x1223 * x1223) * x1220]) (\\[x1225] [x1226, x1227] [x1228] [x1229, x1230] -> let x1231 = cos x1230 ; x1232 = x1231 * x1231 ; x1235 = x1227 * negate (sin x1230) in [x1225, x1225, ((x1235 * x1231 + x1235 * x1231) * negate (recip (x1232 * x1232))) * x1228 + x1225 * recip x1232]) (\\[x1239] [x1240, x1241] [x1242] [x1243, x1244] -> let x1245 = cos x1244 ; x1246 = x1245 * x1245 ; x1252 = negate (recip (x1246 * x1246)) * (x1242 * x1241) in [recip x1246 * x1241 + x1240 + x1239, 0, negate (sin x1244) * (x1245 * x1252 + x1245 * x1252)]) [x1187] [v1193, rreplicate 2 x1188] in let [x1253 @Natural @Double @[], v1254 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) (\\[x1255] [x1256, x1257, x1258] -> let x1259 = cos x1258 in [x1255 + x1256 * recip (x1259 * x1259), x1255]) (\\[x1263] [x1264, x1265, x1266] [x1267] [x1268, x1269, x1270] -> let x1271 = cos x1270 ; x1272 = x1271 * x1271 ; x1276 = x1266 * negate (sin x1270) in [x1263 + x1264 * recip x1272 + ((x1276 * x1271 + x1276 * x1271) * negate (recip (x1272 * x1272))) * x1268, x1263]) (\\[x1281] [x1282] [x1283] [x1284, x1285, x1286] -> let x1287 = cos x1286 ; x1288 = x1287 * x1287 ; x1294 = negate (recip (x1288 * x1288)) * (x1284 * x1281) in [x1281 + x1282, recip x1288 * x1281, 0, negate (sin x1286) * (x1287 * x1294 + x1287 * x1294)]) [x1186] [rreplicate 2 x1185, v1192, rreplicate 2 x1188] in let [x1295 @Natural @Double @[], v1296 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) (\\[x1297] [x1298, x1299, x1300, x1301, x1302] -> let x1303 = cos x1302 ; x1304 = x1303 * x1303 ; x1306 = x1299 * negate (sin x1302) in [x1297, ((x1306 * x1303 + x1306 * x1303) * negate (recip (x1304 * x1304))) * x1300 + x1297 * recip x1304]) (\\[x1310] [x1311, x1312, x1313, x1314, x1315] [x1316] [x1317, x1318, x1319, x1320, x1321] -> let x1322 = cos x1321 ; x1323 = x1322 * x1322 ; x1325 = negate (sin x1321) ; x1326 = x1318 * x1325 ; x1327 = x1326 * x1322 + x1326 * x1322 ; x1328 = x1323 * x1323 ; x1329 = negate (recip x1328) ; x1334 = x1312 * x1325 + ((x1315 * cos x1321) * -1.0) * x1318 ; x1335 = x1315 * negate (sin x1321) ; x1339 = x1335 * x1322 + x1335 * x1322 in [x1310, ((x1334 * x1322 + x1335 * x1326 + x1334 * x1322 + x1335 * x1326) * x1329 + (((x1339 * x1323 + x1339 * x1323) * negate (recip (x1328 * x1328))) * -1.0) * x1327) * x1319 + x1313 * (x1327 * x1329) + x1310 * recip x1323 + (x1339 * negate (recip (x1323 * x1323))) * x1316]) (\\[x1348] [x1349] [x1350] [x1351, x1352, x1353, x1354, x1355] -> let x1356 = cos x1355 ; x1357 = x1356 * x1356 ; x1359 = negate (sin x1355) ; x1360 = x1352 * x1359 ; x1361 = x1360 * x1356 + x1360 * x1356 ; x1362 = x1357 * x1357 ; x1363 = negate (recip x1362) ; x1368 = x1353 * x1349 ; x1369 = negate (recip (x1362 * x1362)) * (-1.0 * (x1361 * x1368)) ; x1370 = x1363 * x1368 ; x1371 = x1356 * x1370 + x1356 * x1370 ; x1372 = negate (recip (x1357 * x1357)) * (x1350 * x1349) + x1357 * x1369 + x1357 * x1369 in [recip x1357 * x1349 + x1348, 0, x1359 * x1371, (x1361 * x1363) * x1349, 0, negate (sin x1355) * (x1356 * x1372 + x1356 * x1372 + x1360 * x1370 + x1360 * x1370) + cos x1355 * (-1.0 * (x1352 * x1371))]) [x1184] [v1254, rreplicate 2 x1185, v1218, v1193, rreplicate 2 x1188] in [rsum v1296, x1295]) (\\[x1374] [x1375] [x1376] [x1377, x1378] -> let [x1380 @Natural @Double @[], v1381 @Natural @Double @[2], v1382 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) (\\[x1383] [x1384] -> [x1383 + tan x1384, x1383, x1383]) (\\[x1386] [x1387] [x1388] [x1389] -> let x1390 = cos x1389 in [x1386 + x1387 * recip (x1390 * x1390), x1386, x1386]) (\\[x1394] [x1395, x1396] [x1397] [x1398] -> let x1399 = cos x1398 in [x1394 + x1396 + x1395, recip (x1399 * x1399) * x1394]) [x1378] [rreplicate 2 x1377] in let [x1406 @Natural @Double @[], v1407 @Natural @Double @[2], v1408 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) (\\[x1409] [x1410, x1411] -> let x1412 = cos x1411 in [x1409, x1409, recip (x1412 * x1412) * x1409]) (\\[x1414] [x1415, x1416] [x1417] [x1418, x1419] -> let x1420 = cos x1419 ; x1421 = x1420 * x1420 ; x1424 = x1416 * negate (sin x1419) in [x1414, x1414, ((x1424 * x1420 + x1424 * x1420) * negate (recip (x1421 * x1421))) * x1417 + x1414 * recip x1421]) (\\[x1428] [x1429, x1430] [x1431] [x1432, x1433] -> let x1434 = cos x1433 ; x1435 = x1434 * x1434 ; x1441 = negate (recip (x1435 * x1435)) * (x1431 * x1430) in [recip x1435 * x1430 + x1429 + x1428, 0, negate (sin x1433) * (x1434 * x1441 + x1434 * x1441)]) [x1376] [v1382, rreplicate 2 x1377] in let [x1446 @Natural @Double @[], v1447 @Natural @Double @[2], v1448 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) (\\[x1449] [x1450, x1451, x1452, x1453] -> let x1454 = cos x1453 ; x1455 = x1454 * x1454 ; x1457 = negate (recip (x1455 * x1455)) * (x1451 * x1450) in [recip x1455 * x1450 + x1449, 0, negate (sin x1453) * (x1454 * x1457 + x1454 * x1457)]) (\\[x1458] [x1459, x1460, x1461, x1462] [x1463] [x1464, x1465, x1466, x1467] -> let x1468 = cos x1467 ; x1469 = x1468 * x1468 ; x1471 = x1469 * x1469 ; x1472 = negate (recip x1471) ; x1473 = x1465 * x1464 ; x1474 = x1472 * x1473 ; x1477 = x1462 * negate (sin x1467) ; x1478 = x1477 * x1468 + x1477 * x1468 ; x1488 = (((x1478 * x1469 + x1478 * x1469) * negate (recip (x1471 * x1471))) * -1.0) * x1473 + (x1460 * x1464 + x1459 * x1465) * x1472 in [x1458 + (x1478 * negate (recip (x1469 * x1469))) * x1464 + x1459 * recip x1469, 0.0, ((x1462 * cos x1467) * -1.0) * (x1468 * x1474 + x1468 * x1474) + (x1477 * x1474 + x1488 * x1468 + x1477 * x1474 + x1488 * x1468) * negate (sin x1467)]) (\\[x1493] [x1494, x1495] [x1496] [x1497, x1498, x1499, x1500] -> let x1501 = cos x1500 ; x1502 = x1501 * x1501 ; x1504 = x1502 * x1502 ; x1505 = negate (recip x1504) ; x1506 = x1498 * x1497 ; x1507 = x1505 * x1506 ; x1513 = negate (sin x1500) * x1495 ; x1514 = x1501 * x1513 + x1501 * x1513 ; x1515 = x1505 * x1514 ; x1516 = negate (recip (x1504 * x1504)) * (-1.0 * (x1506 * x1514)) ; x1517 = negate (recip (x1502 * x1502)) * (x1497 * x1493) + x1502 * x1516 + x1502 * x1516 in [x1493, x1498 * x1515 + recip x1502 * x1493, x1497 * x1515, 0, negate (sin x1500) * (x1501 * x1517 + x1501 * x1517 + x1507 * x1513 + x1507 * x1513) + cos x1500 * (-1.0 * ((x1501 * x1507 + x1501 * x1507) * x1495))]) [x1375] [rreplicate 2 x1374, v1407, v1382, rreplicate 2 x1377] in let [x1520 @Natural @Double @[], v1521 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) (\\[x1522] [x1523, x1524, x1525] -> let x1526 = cos x1525 in [x1522 + x1523, recip (x1526 * x1526) * x1522]) (\\[x1528] [x1529, x1530, x1531] [x1532] [x1533, x1534, x1535] -> let x1536 = cos x1535 ; x1537 = x1536 * x1536 ; x1540 = x1531 * negate (sin x1535) in [x1528 + x1529, ((x1540 * x1536 + x1540 * x1536) * negate (recip (x1537 * x1537))) * x1532 + x1528 * recip x1537]) (\\[x1544] [x1545] [x1546] [x1547, x1548, x1549] -> let x1550 = cos x1549 ; x1551 = x1550 * x1550 ; x1555 = negate (recip (x1551 * x1551)) * (x1546 * x1545) in [x1544 + recip x1551 * x1545, x1544, 0, negate (sin x1549) * (x1550 * x1555 + x1550 * x1555)]) [rfromS 0.0] [v1447, v1381, rreplicate 2 x1377] in [x1446, rsum v1521 + rsum v1448, x1520]) [1.0] [v991, rreplicate 2 1.1] in [rsum v1133 + x1132]"
