...
testSin0Fold182SrevPP = do
  ...
  let a1 = rrev1 @(AstRanked FullSpan)
           (let f :: forall f. ADReadyS f => f Double '[] -> f Double '[5]
            ...
            in rfromS . f . sfromR) 1.1
  printAstPretty IM.empty a1
    @?= "rfromS (let m123 = sscanDer <lambda> <lambda> <lambda> (sreplicate 1.1) (sreplicate 1.1) in sfromR (let [v124 @[Natural] @Double @[5], v125 @[Natural] @Double @[1]] = dmapAccumRDer (SNat @1) <lambda> <lambda> <lambda> [sfromR (rreplicate 5 1.0)] [sslice m123, sreplicate 1.1] in rfromS (ssum v124) + rfromS (ssum v125)))"
...
testSin0rmapAccumRD01SN531b0PP = do
  ...
  printAstHVectorPrettyButNested
    ...
    @?= "let v58 = rconst (fromList [0] []) in let [x59 @[Natural] @Double @[], v60 @[Natural] @Double @[0]] = dmapAccumRDer (SNat @0) (\\[x61] [x62] -> [x61, x61]) (\\[x63] [x64] [x65] [x66] -> [x63, x63]) (\\[x67] [x68] [x69] [x70] -> [0.0 + x67 + x68, 0]) [1.1] [v58] in let [x74 @[Natural] @Double @[], v75 @Natural @Double @[0]] = dmapAccumLDer (SNat @0) (\\[x76] [x77, x78] -> [x76, 0]) (\\[x79] [x80, x81] [x82] [x83, x84] -> [x79, 0.0]) (\\[x85] [x86] [x87] [x88, x89] -> [x85, 0, 0]) [sfromR (rsum (rreplicate 4 1.0))] [v60, v58] in [rfromS x74]"
...
testSin0rmapAccumRD01SN531bSPP = do
  ...
  printAstHVectorPretty
    ...
    @?= "let v58 = sconst @[1] (fromList @[1] [0.0]) in let [x59 @[Natural] @Double @[], v60 @[Natural] @Double @[1]] = dmapAccumRDer (SNat @1) <lambda> <lambda> <lambda> [1.1] [v58] in let [x74 @[Natural] @Double @[], v75 @[Natural] @Double @[1]] = dmapAccumLDer (SNat @1) <lambda> <lambda> <lambda> [4.0] [v60, v58] in [x74]"
  ...
  let a1 = rrev1 @(AstRanked FullSpan) @Double @0 @1
                 (\x0 -> rscanZip (\x _a -> sin x)
                           ...
                           x0 (V.singleton $ DynamicRanked
                               $ rconst (OR.constant @Double @1 [2] 42))) 1.1
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v97 = rconst (fromList [2] [42.0,42.0]) in let [x98 @Natural @Double @[], v99 @Natural @Double @[2], v100 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) <lambda> <lambda> <lambda> [1.1] [v97] in let [x121 @Natural @Double @[], v122 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) <lambda> <lambda> <lambda> [0.0] [rreplicate 2 1.0, v99, v97] in x121 + 1.0"
  ...
  let a1 = rrev1 @(AstRanked FullSpan) @Double @0 @1
                 (\x0 -> rscanZip (\x a -> sin x - rfromD (a V.! 0))
                         ...
                         x0 (V.singleton $ DynamicRanked
                             $ rconst (OR.fromList @Double @1 [2] [5, 7]))) 1.1
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v105 = rconst (fromList [2] [5.0,7.0]) in let [x106 @Natural @Double @[], v107 @Natural @Double @[2], v108 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) <lambda> <lambda> <lambda> [1.1] [v105] in let [x131 @Natural @Double @[], v132 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) <lambda> <lambda> <lambda> [0.0] [rreplicate 2 1.0, v107, v105] in x131 + 1.0"
