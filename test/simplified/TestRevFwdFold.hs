...
testSin0Fold182SrevPP = do
  ...
  let a1 = rrev1 @(AstRanked FullSpan)
           (let f :: forall f. ADReadyS f => f Double '[] -> f Double '[5]
            ...
            in rfromS . f . sfromR) 1.1
  printAstPretty IM.empty a1
    @?= "let m123 = sscanDer <lambda> <lambda> <lambda> (sreplicate (sconstant (rconst 1.1))) (sreplicate (sconstant (rconst 1.1))) in let [v124 @[Natural] @Double @[5], v125 @[Natural] @Double @[1]] = dmapAccumRDer (SNat @1) <lambda> <lambda> <lambda> [rreplicate 5 (rconst 1.0)] [sslice m123, sreplicate (sconstant (rconst 1.1))] in ssum v124 + ssum v125"
...
testSin0Scan1RevPP1 = do
  ...
  printAstPrettyButNested IM.empty (simplifyAst6 a1)
    @?= "let v56 = rconst (fromList [2] [42.0,42.0]) in let [x67 @Natural @Double @[], v68 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) (\\[x69] [x70, x71, x72] -> [cos x71 * (x69 + x70), 0]) (\\[x73] [x74, x75, x76] [x77] [x78, x79, x80] -> [(x75 * negate (sin x79)) * (x77 + x78) + (x73 + x74) * cos x79, rconst 0.0]) (\\[x86] [x87] [x88] [x89, x90, x91] -> let x96 = cos x90 * x86 in [x96, x96, negate (sin x90) * ((x88 + x89) * x86), 0]) [rconst 0.0] [rconstant (rreplicate 2 (rconst 1.0)), rslice 0 2 (rscanDer (\\x57 x58 -> sin x57) (\\x59 x60 x61 x62 -> x59 * cos x61) (\\x63 x64 x65 -> [cos x64 * x63, 0]) (rconst 1.1) v56), v56] in x67 + rconst 1.0"
...
testSin0ScanFwdPP = do
  ...
  let a1 = rfwd1 @(AstRanked FullSpan) @Double @0 @1
                 (\x0 -> rscan (\x _a -> sin x) x0
                           (rconst (OR.constant @Double @1 [2] 42))) 1.1
  printAstPrettyButNested IM.empty (simplifyAst6 a1)
    @?= "let v60 = rconst (fromList [2] [42.0,42.0]) in let [x71 @Natural @Double @[], v72 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) (\\[x73] [x74, x75, x76] -> let x77 = x73 * cos x75 in [x77, x77]) (\\[x78] [x79, x80, x81] [x82] [x83, x84, x85] -> let x89 = x78 * cos x84 + (x80 * negate (sin x84)) * x82 in [x89, x89]) (\\[x90] [x91] [x92] [x93, x94, x95] -> let x100 = x91 + x90 in [cos x94 * x100, 0, negate (sin x94) * (x92 * x100), 0]) [rconst 1.1] [rconstant (rreplicate 2 (rconst 0.0)), rslice 0 2 (rscanDer (\\x61 x62 -> sin x61) (\\x63 x64 x65 x66 -> x63 * cos x65) (\\x67 x68 x69 -> [cos x68 * x67, 0]) (rconst 1.1) v60), v60] in rappend (rconstant (rreplicate 1 (rconst 1.1))) v72"
...
testSin0Scan1Rev2PP1 = do
  ...
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v63 = rconst (fromList [2] [5.0,7.0]) in let [x75 @Natural @Double @[], v76 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) <lambda> <lambda> <lambda> [rconst 0.0] [rconstant (rreplicate 2 (rconst 1.0)), rslice 0 2 (rscanDer <lambda> <lambda> <lambda> (rconst 1.1) v63), v63] in x75 + rconst 1.0"
...
testSin0Scan1Rev2PPA = do
  ...
  printAstHVectorPretty IM.empty (simplifyAstHVector6 a1)
    @?= "let v15 = rconst (fromList [2] [5.0,7.0]) in let [x60 @Natural @Double @[], v61 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) <lambda> <lambda> <lambda> [rconst 0.0] [rslice 1 2 v26, rslice 0 2 (rscanDer <lambda> <lambda> <lambda> x1 v15), v15] in [x60 + v26 ! [0]]"
...
testSin0Scan1Rev2PPForComparison = do
  ...
  printAstHVectorPretty IM.empty (simplifyAstHVector6 a1)
    @?= "[cos x1 * (cos (sin x1 - rconst 5.0) * v3 ! [0]) + cos x1 * v3 ! [1] + v3 ! [2]]"
  ...
  let ((_, a1, _), _) =
        fwdArtifactAdapt @Double @1 @(AstRanked FullSpan)
                 (\x0 -> rscan (\x a -> sin x - a) x0
                           (rconst (OR.fromList @Double @1 [2] [5, 7]))) 1.1
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v16 = rconst (fromList [2] [5.0,7.0]) in let [x27 @Natural @Double @[], v28 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) <lambda> <lambda> <lambda> [x1] [rreplicate 2 (rconst 0.0), rslice 0 2 (rscanDer <lambda> <lambda> <lambda> x2 v16), v16] in rappend (rreplicate 1 x1) v28"
...
testSin0Scan1Rev3PP = do
  ...
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v63 = rfromList [rconst 1.1 * rconst 5.0, rconst 1.1 * rconst 7.0] in let [x75 @Natural @Double @[], v76 @Natural @Double @[2]] = dmapAccumRDer (SNat @2) <lambda> <lambda> <lambda> [rconst 0.0] [rconstant (rreplicate 2 (rconst 1.0)), rslice 0 2 (rscanDer <lambda> <lambda> <lambda> (rconst 1.1) v63), v63] in rconst 5.0 * v76 ! [0] + rconst 7.0 * v76 ! [1] + x75 + rconst 1.0"
...
testSin0ScanFwd3PP = do
  ...
  let a1 = rfwd1 @(AstRanked FullSpan) @Double @0 @1
                 (\x0 -> rscan (\x a -> sin x - a) x0
                           (rfromList [x0 * 5, x0 * 7])) 1.1
  printAstPretty IM.empty (simplifyAst6 a1)
    @?= "let v72 = rfromList [rconst 1.1 * rconst 5.0, rconst 1.1 * rconst 7.0] in let [x87 @Natural @Double @[], v88 @Natural @Double @[2]] = dmapAccumLDer (SNat @2) <lambda> <lambda> <lambda> [rconst 1.1] [rfromList [rconst 1.1 * rconst 5.0, rconst 1.1 * rconst 7.0], rslice 0 2 (rscanDer <lambda> <lambda> <lambda> (rconst 1.1) v72), v72] in rappend (rconstant (rreplicate 1 (rconst 1.1))) v88"
...
testSin0rmapAccumRD01SN531b0PP = do
  ...
  printAstHVectorPrettyButNested
    ...
    @?= "let v58 = rconst (fromList [0] []) in let [x59 @[Natural] @Double @[], v60 @[Natural] @Double @[0]] = dmapAccumRDer (SNat @0) (\\[x61] [x62] -> [x61, x61]) (\\[x63] [x64] [x65] [x66] -> [x63, x63]) (\\[x67] [x68] [x69] [x70] -> [sconst @[] 0.0 + x67 + x68, 0]) [sconstant (rconst 1.1)] [v58] in let [x74 @[Natural] @Double @[], v75 @Natural @Double @[0]] = dmapAccumLDer (SNat @0) (\\[x76] [x77, x78] -> [x76, 0]) (\\[x79] [x80, x81] [x82] [x83, x84] -> [x79, rconst 0.0]) (\\[x85] [x86] [x87] [x88, x89] -> [x85, 0, 0]) [sconstant (rsum (rreplicate 4 (rconst 1.0)))] [v60, v58] in [x74]"
...
testSin0rmapAccumRD01SN531bSPP = do
  ...
  printAstHVectorPretty
    ...
    @?= "let v58 = sconst @[1] (fromList @[1] [0.0]) in let [x59 @[Natural] @Double @[], v60 @[Natural] @Double @[1]] = dmapAccumRDer (SNat @1) <lambda> <lambda> <lambda> [sconst @[] 1.1] [v58] in let [x74 @[Natural] @Double @[], v75 @[Natural] @Double @[1]] = dmapAccumLDer (SNat @1) <lambda> <lambda> <lambda> [sconstant (rsum (rgather [4] (sconst @[2,2] (fromList @[2,2] [1.0,1.0,1.0,1.0])) (\\[i92] -> [rem (quot i92 2) 2, rem i92 2])))] [v60, v58] in [x74]"
...
testSin0rmapAccumRD01SN531bRPP = do
  ...
  printAstHVectorSimple
    ...
    (simplifyAstHVector6
     $ g @(AstRanked FullSpan) (V.singleton $ DynamicRanked @Double @0 1.1))
    @?= "rletInHVector (rconst (fromList [1] [0.0])) (\\v58 -> dletHVectorInHVector (dmapAccumRDer (SNat @1) (\\[x61 @Natural @Double @[]] [x62 @Natural @Double @[]] -> dmkHVector (fromList [DynamicRanked x61, DynamicRanked x61])) (\\[x63 @Natural @Double @[]] [x64 @Natural @Double @[]] [x65 @Natural @Double @[]] [x66 @Natural @Double @[]] -> dmkHVector (fromList [DynamicRanked x63, DynamicRanked x63])) (\\[x67 @Natural @Double @[]] [x68 @Natural @Double @[]] [x69 @Natural @Double @[]] [x70 @Natural @Double @[]] -> dmkHVector (fromList [DynamicRanked (x68 + x67), DynamicRankedDummy])) (fromList [DynamicRanked (rconst 1.1)]) (fromList [DynamicRanked v58])) (\\[x59 @Natural @Double @[], v60 @Natural @Double @[1]] -> dletHVectorInHVector (dmapAccumLDer (SNat @1) (\\[x76 @Natural @Double @[]] [x77 @Natural @Double @[], x78 @Natural @Double @[]] -> dmkHVector (fromList [DynamicRanked x76, DynamicRankedDummy])) (\\[x79 @Natural @Double @[]] [x80 @Natural @Double @[], x81 @Natural @Double @[]] [x82 @Natural @Double @[]] [x83 @Natural @Double @[], x84 @Natural @Double @[]] -> dmkHVector (fromList [DynamicRanked x79, DynamicRanked (rconst 0.0)])) (\\[x85 @Natural @Double @[]] [x86 @Natural @Double @[]] [x87 @Natural @Double @[]] [x88 @Natural @Double @[], x89 @Natural @Double @[]] -> dmkHVector (fromList [DynamicRanked x85, DynamicRankedDummy, DynamicRankedDummy])) (fromList [DynamicRanked (rconstant (rsum (rreplicate 4 (rconst 1.0))))]) (fromList [DynamicRanked v60, DynamicRanked v58])) (\\[x74 @Natural @Double @[], v75 @Natural @Double @[1]] -> dmkHVector (fromList [DynamicRanked x74]))))"
...
testSin0rmapAccumRD01SN531b0PPj = do
  ...
  printAstHVectorPretty
    ...
    (simplifyAstHVector6
     $ g @(AstRanked FullSpan) (V.singleton $ DynamicRanked @Double @0 1.1))
    @?= "let t143 = rconstant (rtranspose [2,0,1] (rreplicate 2 (rreplicate 2 (rconst (fromList [0] []))))) in let [m144 @[Natural] @Double @[2,2], t145 @[Natural] @Double @[0,2,2]] = dmapAccumRDer (SNat @0) <lambda> <lambda> <lambda> [sconstant (sfromIntegral (rtranspose [1,0] (rreplicate 2 (rconst (fromList [2] [0,1]))) + rreplicate 2 (rconst (fromList [2] [0,1])))) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0]) + sconstant (sreplicate (sconst @[2] (fromList @[2] [0.0,0.0]))) + sconstant (sreplicate (sreplicate (rconst 1.1))) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0]) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0]) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0])] [t143] in let [m180 @[Natural] @Double @[2,2], t181 @Natural @Double @[0,2,2]] = dmapAccumLDer (SNat @0) <lambda> <lambda> <lambda> [sconstant (sscatter (rreplicate 2 (rreplicate 2 (rconst 1.0))) (\\[i179] -> [i179]))] [t145, t143] in [ssum (ssum m180)]"
...
testSin0rmapAccumRD01SN531bSPPj = do
  ...
  printAstHVectorPretty
    ...
    (simplifyAstHVector6
     $ g @(AstRanked FullSpan) (V.singleton $ DynamicShaped @Double @'[] 1.1))
    @?= "let t144 = sconstant (stranspose (sreplicate (sreplicate (sconst @[1] (fromList @[1] [0.0]))))) in let [m145 @[Natural] @Double @[2,2], t146 @[Natural] @Double @[1,2,2]] = dmapAccumRDer (SNat @1) <lambda> <lambda> <lambda> [sconstant (sfromIntegral (rtranspose [1,0] (rreplicate 2 (rconst (fromList [2] [0,1]))) + rreplicate 2 (rconst (fromList [2] [0,1])))) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0]) + sconstant (sreplicate (sconst @[2] (fromList @[2] [0.0,0.0]))) + sconstant (sreplicate (sreplicate (sconst @[] 1.1))) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0]) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0]) + sconst @[2,2] (fromList @[2,2] [0.0,0.0,0.0,0.0])] [t144] in let [m181 @[Natural] @Double @[2,2], t182 @[Natural] @Double @[1,2,2]] = dmapAccumLDer (SNat @1) <lambda> <lambda> <lambda> [sconstant (sscatter (sconst @[2,2] (fromList @[2,2] [1.0,1.0,1.0,1.0])) (\\[i180] -> [i180]))] [t146, t144] in [ssum (ssum m181)]"
...
testSin0rmapAccumRD01SN531bRPPj = do
  ...
  printAstHVectorPretty
    ...
    (simplifyAstHVector6
     $ g @(AstRanked FullSpan) (V.singleton $ DynamicRanked @Double @0 1.1))
    @?= "let t61 = rconstant (rtranspose [2,0,1] (rreplicate 2 (rreplicate 2 (rconst (fromList [1] [0.0]))))) in let [m62 @Natural @Double @[2,2], t63 @Natural @Double @[1,2,2]] = dmapAccumRDer (SNat @1) <lambda> <lambda> <lambda> [rconstant (rfromIntegral (rtranspose [1,0] (rreplicate 2 (rconst (fromList [2] [0,1]))) + rreplicate 2 (rconst (fromList [2] [0,1])))) + rconstant (rreplicate 2 (rreplicate 2 (rconst 1.1)))] [t61] in let [m77 @Natural @Double @[2,2], t78 @Natural @Double @[1,2,2]] = dmapAccumLDer (SNat @1) <lambda> <lambda> <lambda> [rconstant (rreplicate 2 (rreplicate 2 (rconst 1.0)))] [t63, t61] in [rsum (rreshape [4] m77)]"
...
testSin0FoldNestedS1PP = do
  ...
  printAstHVectorPretty
    IM.empty
    (g @(AstRanked FullSpan) (V.singleton $ DynamicShaped @Double @'[] 1.1))
    @?= "let v920 = sscanDer <lambda> <lambda> <lambda> (sconst @[] 1.1) (sreplicate (sconst @[] 1.1)) in let [x921 @[Natural] @Double @[], v922 @[Natural] @Double @[11]] = dmapAccumRDer (SNat @11) <lambda> <lambda> <lambda> [sconst @[] 1.0] [sslice v920, sreplicate (sconst @[] 1.1)] in [x921 + sconst @[] 0.0 + ssum v922]"
...
testSin0FoldNestedR1PP = do
  ...
  printAstHVectorPretty
    IM.empty
    (g @(AstRanked FullSpan) (V.singleton $ DynamicRanked @Double @0 1.1))
    @?= "let v921 = rscanDer <lambda> <lambda> <lambda> (rconst 1.1) (rreplicate 11 (rconst 1.1)) ; x922 = rreshape [] (rreplicate 1 (rconst 1.0)) in let [x923 @Natural @Double @[], v924 @Natural @Double @[11]] = dmapAccumRDer (SNat @11) <lambda> <lambda> <lambda> [x922] [rslice 0 11 v921, rreplicate 11 (rconst 1.1)] in [rsum v924 + x923]"
...
testSin0FoldNestedR1SimpPP = do
  ...
  printAstHVectorPretty
    ...
    (simplifyAstHVector6
     $ g @(AstRanked FullSpan) (V.singleton $ DynamicRanked @Double @0 1.1))
    @?= "let [x923 @Natural @Double @[], v924 @Natural @Double @[11]] = dmapAccumRDer (SNat @11) <lambda> <lambda> <lambda> [rconst 1.0] [rslice 0 11 (rscanDer <lambda> <lambda> <lambda> (rconst 1.1) (rconstant (rreplicate 11 (rconst 1.1)))), rconstant (rreplicate 11 (rconst 1.1))] in [rsum v924 + x923]"
