...
testVT2OPP = do
  ...
  printGradient6Simple renames (simplifyArtifact6 artifact6)
    @?= "\\s0 dret x3 x4 x5 x6 x7 x8 -> dlet (tconstant (tkonst 2 (tkonst 784 (tconst 7.0)))) (\\x12 -> dlet (tkonst 3 (tsum (tgather [784,2] x12 (\\[i26, i27] -> [i27, i26]) * tgather [784,2] x3 (\\[i26, i27] -> [i27, i26])) + x4)) (\\x13 -> dlet (tkonst 4 (tsum (tgather [2,3] x13 (\\[i28, i29] -> [i29, i28]) * tgather [2,3] x5 (\\[i28, i29] -> [i29, i28])) + x6)) (\\x14 -> dlet (tgather [4,3] dret (\\[i20, i21] -> [i20])) (\\x15 -> dlet (tsum (x7 * x15)) (\\x16 -> dlet (tgather [1,2] x16 (\\[i22, i23] -> [i22])) (\\x17 -> dlet (tsum (x5 * x17)) (\\x18 -> dlet (tgather [1,784] x18 (\\[i24, i25] -> [i24])) (\\x19 -> dmkDomains (fromList [dfromR (tfromList []), dfromR (x12 * x19), dfromR x18, dfromR (x13 * x17), dfromR x16, dfromR (x14 * x15), dfromR dret])))))))))"
