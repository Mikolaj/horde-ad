...
testVTOPPNonLin = do
  ...
  printGradient6Pretty renames (simplifyArtifact6 artifact6nonLin)
    @?= "\\s0 dret x3 x4 x5 x6 x7 x8 x9 x10 x11 x12 x13 x14 x15 x16 x17 -> let x26 = tconstant (tkonst 784 (tconst 7.0)) ; x30 = recip (tconstant (tkonst 3 (tconst 1.0)) + exp (negate (tfromList [tsum (x3 * x26), tsum (x4 * x26), tsum (x5 * x26)] + x6))) ; x36 = recip (tconstant (tkonst 4 (tconst 1.0)) + exp (negate (tfromList [tsum (x7 * x30), tsum (x8 * x30), tsum (x9 * x30), tsum (x10 * x30)] + x11))) ; x39 = exp (tfromList [tsum (x12 * x36), tsum (x13 * x36), tsum (x14 * x36), tsum (x15 * x36), tsum (x16 * x36)] + x17) ; x40 = tsum x39 ; x42 = x39 * (tkonst 5 (negate (recip (x40 * x40)) * tsum (x39 * dret)) + tkonst 5 (recip x40) * dret) ; x43 = x42 ! [4] ; x44 = x42 ! [3] ; x45 = x42 ! [2] ; x46 = x42 ! [1] ; x47 = x42 ! [0] ; x50 = (x36 * (tconstant (tkonst 4 (tconst 1.0)) - x36)) * (x12 * tkonst 5 x47 + x13 * tkonst 5 x46 + x14 * tkonst 5 x45 + x15 * tkonst 5 x44 + x16 * tkonst 5 x43) ; x51 = x50 ! [3] ; x52 = x50 ! [2] ; x53 = x50 ! [1] ; x54 = x50 ! [0] ; x57 = (x30 * (tconstant (tkonst 3 (tconst 1.0)) - x30)) * (x7 * tkonst 4 x54 + x8 * tkonst 4 x53 + x9 * tkonst 4 x52 + x10 * tkonst 4 x51) in (tfromList [], x26 * tkonst 784 (x57 ! [0]), x26 * tkonst 784 (x57 ! [1]), x26 * tkonst 784 (x57 ! [2]), x57, x30 * tkonst 3 x54, x30 * tkonst 3 x53, x30 * tkonst 3 x52, x30 * tkonst 3 x51, x50, x36 * tkonst 4 x47, x36 * tkonst 4 x46, x36 * tkonst 4 x45, x36 * tkonst 4 x44, x36 * tkonst 4 x43, x42)"
...
testVT2OPP = do
  ...
  printGradient6Pretty renames (simplifyArtifact6 artifact6)
    @?= "\\s0 dret x3 x4 x5 x6 x7 x8 -> let x12 = tconstant (tkonst 3 (tkonst 784 (tconst 7.0))) ; x13 = tkonst 4 (tsum (ttranspose [1,0] (x12 * x3)) + x4) ; x15 = ttranspose [1,0] (tkonst 4 dret) ; x16 = tsum (x7 * x15) ; x17 = ttranspose [1,0] (tkonst 3 x16) ; x18 = tsum (x5 * x17) in (tfromList [], x12 * ttranspose [1,0] (tkonst 784 x18), x18, x13 * x17, x16, tkonst 5 (tsum (ttranspose [1,0] (x13 * x5)) + x6) * x15, dret)"
  printPrimal6Pretty renames (simplifyArtifact6 artifact6)
...
testVT2OPPNonLin = do
  ...
  printGradient6Pretty renames (simplifyArtifact6 artifact6nonLin)
    @?= "\\s0 dret x3 x4 x5 x6 x7 x8 -> let x17 = tconstant (tkonst 3 (tkonst 784 (tconst 7.0))) ; x21 = recip (tconstant (tkonst 3 (tconst 1.0)) + exp (negate (tsum (ttranspose [1,0] (x17 * x3)) + x4))) ; x24 = tkonst 4 x21 ; x28 = recip (tconstant (tkonst 4 (tconst 1.0)) + exp (negate (tsum (ttranspose [1,0] (x24 * x5)) + x6))) ; x31 = tkonst 5 x28 ; x32 = exp (tsum (ttranspose [1,0] (x31 * x7)) + x8) ; x33 = tsum x32 ; x35 = x32 * (tkonst 5 (negate (recip (x33 * x33)) * tsum (x32 * dret)) + tkonst 5 (recip x33) * dret) ; x36 = ttranspose [1,0] (tkonst 4 x35) ; x39 = (x28 * (tconstant (tkonst 4 (tconst 1.0)) - x28)) * tsum (x7 * x36) ; x40 = ttranspose [1,0] (tkonst 3 x39) ; x43 = (x21 * (tconstant (tkonst 3 (tconst 1.0)) - x21)) * tsum (x5 * x40) in (tfromList [], x17 * ttranspose [1,0] (tkonst 784 x43), x43, x24 * x40, x39, x31 * x36, x35)"
  printPrimal6Pretty renames (simplifyArtifact6 artifact6nonLin)
    @?= "\\s0 x3 x4 x5 x6 x7 x8 -> let x32 = exp (tsum (ttranspose [1,0] (tkonst 5 (recip (tconstant (tkonst 4 (tconst 1.0)) + exp (negate (tsum (ttranspose [1,0] (tkonst 4 (recip (tconstant (tkonst 3 (tconst 1.0)) + exp (negate (tsum (ttranspose [1,0] (tconstant (tkonst 3 (tkonst 784 (tconst 7.0))) * x3)) + x4)))) * x5)) + x6)))) * x7)) + x8) in tkonst 5 (recip (tsum x32)) * x32"
