...
testVT2OPP = do
  ...
  printPrimal6Pretty renames (simplifyArtifact6 artifact6)
    @?= "\\s0 x3 x4 x5 x6 x7 x8 -> tsum ((tsum ((tsum (tconstant (tkonst 784 (tkonst 3 (tkonst 4 (tkonst 5 (tconst 7.0))))) * tgather [784,3,4,5] x3 (\\[i53, i54, i55, i56] -> [i54, i53])) + tgather [3,4,5] x4 (\\[i37, i38, i39] -> [i37])) * tgather [3,4,5] x5 (\\[i37, i38, i39] -> [i38, i37])) + tgather [4,5] x6 (\\[i26, i27] -> [i26])) * ttranspose [1,0] x7) + x8"
...
testVT2OPPNonLin = do
  ...
  printPrimal6Pretty renames (simplifyArtifact6 artifact6nonLin)
    @?= "\\s0 x3 x4 x5 x6 x7 x8 -> let x21 = recip (tconstant (tkonst 3 (tconst 1.0)) + exp (negate (tsum (tconstant (tkonst 784 (tkonst 3 (tconst 7.0))) * ttranspose [1,0] x3) + x4))) ; x28 = recip (tconstant (tkonst 4 (tconst 1.0)) + exp (negate (tsum (tgather [3,4] x21 (\\[i53, i54] -> [i53]) * ttranspose [1,0] x5) + x6))) ; x32 = exp (tsum (tgather [4,5] x28 (\\[i55, i56] -> [i55]) * ttranspose [1,0] x7) + x8) in tkonst 5 (recip (tsum x32)) * x32"
