...
gdTestsRecord = testGroup "Record of shaped tensors tests"
  [ testCase "0.1 30" $ do
      let res = gdShowRecord 0.1 (fquadRecord @1) [[2], [3]] 30
      res @?~ ([2.47588e-3,3.7138206e-3],5.00002 :: Float)
  , testCase "0.01 30" $ do
      let res = gdShowRecord 0.01 (fquadRecord @1) [[2], [3]] 30
      res @?~ ([1.0909687,1.6364527],8.86819 :: Float)
  , testCase "0.01 300" $ do
      let res = gdShowRecord 0.01 (fquadRecord @1) [[2], [3]] 300
      res @?~ ([4.665013e-3,6.9975173e-3],5.0000706 :: Float)
  , testCase "0.01 300000" $ do
      let res = gdShowRecord 0.01 (fquadRecord @1) [[2], [3]] 300000
      res @?~ ([3.5e-44,3.5e-44],5.0 :: Float)
  , testCase "0.1 30" $ do
      let res = gdShowRecord 0.1 (fquadRecord @2) [[2, 42], [3, 42]] 30
      res @?~ ([2.47588e-3,42,3.7138206e-3,42],5.00002 :: Float)
  , testCase "0.01 30" $ do
      let res = gdShowRecord 0.01 (fquadRecord @2) [[2, 42], [3, 42]] 30
      res @?~ ([1.0909687,42,1.6364527,42],8.86819 :: Float)
  , testCase "0.01 300" $ do
      let res = gdShowRecord 0.01 (fquadRecord @2) [[2, 42], [3, 42]] 300
      res @?~ ([4.665013e-3,42,6.9975173e-3,42],5.0000706 :: Float)
  , testCase "0.01 300000" $ do
      let res = gdShowRecord 0.01 (fquadRecord @2) [[2, 42], [3, 42]] 300000
