...
reluS, reluLeakyS
  :: forall shaped sh r.
     ( Sh.Shape sh, KnownNat (Sh.Rank sh), ADReadyS shaped, GoodScalar r
     , Differentiable r )
...
logisticS :: forall shaped r sh.
             ( Sh.Shape sh, ShapedTensor shaped, GoodScalar r
...
squareS :: forall shaped r sh.
           ( Sh.Shape sh, ShapedTensor shaped, Num (PrimalOf shaped r sh)
           , GoodScalar r )
       => shaped r sh -> shaped r sh
...
lossCrossEntropyVS :: ( Sh.Shape sh, KnownNat (Sh.Size sh)
...
softMax1S :: ( Sh.Shape sh, KnownNat (Sh.Size sh)
             , ShapedTensor shaped, GoodScalar r, Differentiable r )
          => shaped r sh -> shaped r sh
softMax1S d =
  let expU0 = exp d
...
conv2dUnpaddedS arrK arrA =
  sbuild @shaped @r @(Sh.Rank shB) $ \case
    [iImg, iCout, iBh, iBw] ->
...
slicezS
  :: forall shOut sh shaped r.
     ( Sh.Shape sh, Sh.Shape shOut, Sh.Shape (Sh.Take (Sh.Rank sh) shOut)
     , KnownNat (Sh.Rank sh)
     , Sh.Rank shOut ~ Sh.Rank sh, ADReadyS shaped, GoodScalar r )
  => shaped r sh -> IndexSh shaped sh -> shaped r shOut
slicezS d ixBase =
  gcastWith (unsafeCoerce Refl
             :: Sh.Rank (Sh.Take (Sh.Rank shOut) shOut) :~: Sh.Rank shOut) $
  gcastWith (unsafeCoerce Refl :: Sh.Drop (Sh.Rank sh) shOut :~: '[]) $
...
indexz0SLet
  :: forall shOut sh shaped r.
     ( Sh.Shape shOut, KnownNat (Sh.Rank shOut), Sh.Shape sh
     , ADReadyS shaped, GoodScalar r )
...
indexz0S
  :: forall shOut sh shaped r.
     (Sh.Shape shOut, Sh.Shape sh, ADReadyS shaped, GoodScalar r)
  => shaped r sh -> IndexOf shaped (Sh.Rank shOut) -> shaped r '[]
indexz0S d ix =
  ifF (within0S @shOut @shaped ix)
      (sindex0 d (ShapedList.listToIndex (indexToList ix)))
...
within0S
  :: forall shOut shaped. (Sh.Shape shOut, ADReadyS shaped)
...
within0S ix =
  ...
  in foldr (&&*) true
     $ zipWith within (indexToList ix) (map fromIntegral $ Sh.shapeT @shOut)
...
maxPool2dUnpaddedS arr =
  ...
  in sbuild @shaped @r @(Sh.Rank shOut) $ \case
    [iImg, iChan, iBh, iBw] ->
