  ...
  trcast @r1 @r2 a =
    ...
    in case typeRep @r1 of
      ...
      Is @Float -> case typeRep @r2 of
        ...
        _ -> cast a
      _ -> cast a
  trargMin = Concrete . targMinR . unConcrete
  trargMax = Concrete . targMaxR . unConcrete
  {-# INLINE triota #-}
...
tindexZRSlow :: forall m n x. KnownSTK x
             => Concrete (TKR2 (m + n) x) -> IxROf Concrete m
             -> Concrete (TKR2 n x)
{-# NOINLINE tindexZRSlow #-}  -- the rare slow case
tindexZRSlow (Concrete v) ix | Dict <- eltDictRep (knownSTK @x) =
  if ixInBoundsR (shrTakeIx @m @n Proxy ix (Nested.rshape v))  ix
  then Concrete $ Nested.rindexPartial v (fmapUnConcrete ix)
  else case tftkG (STKR (Nested.rrank v) (knownSTK @x)) v of
         FTKR _ x -> tdefTarget (FTKR (shrDropIx @m @n ix (Nested.rshape v)) x)

-- See the comment about tscatterZSDict.
tindexZRDict :: forall m n r. KnownNat n
             => Dict GoodScalar r
             -> Concrete (TKR (m + n) r)
             -> IxROf Concrete m
             -> Concrete (TKR n r)
...
tindexZRScalar :: forall m n r. (KnownNat n, GoodScalar r)
...
tindexZRScalar (Concrete v) ix = case SNat @n of
  ...
  _ ->
    ...
    $ if ixInBoundsR (shrTakeIx @m @n Proxy ix (Nested.rshape v)) ix
      ...
      else Nested.rreplicatePrim (shrDropIx @m @n ix (Nested.rshape v)) def
...
tgatherZR
  :: forall m n p x. (KnownNat m, KnownNat n, KnownSTK x)
...
tgatherZRScalar
  :: forall m n p r. (KnownNat m, KnownNat n, GoodScalar r)
  => IShR m -> Concrete (TKR (p + n) r)
  -> (IxROf Concrete m -> IxROf Concrete p)
  -> Concrete (TKR (m + n) r)
...
tgatherZ1RSlow
  :: forall n p x. (KnownNat n, KnownSTK x)
  => Int -> Concrete (TKR2 (p + n) x)
  -> (IntOf Concrete -> IxROf Concrete p)
  -> Concrete (TKR2 (1 + n) x)
...
{-# INLINE targMinR #-}
targMinR v | SNat <- Nested.rrank v =
...
targMaxR
  ...
  => Nested.Ranked (1 + n) r -> Nested.Ranked n Int
{-# INLINE targMaxR #-}
targMaxR v | SNat <- Nested.rrank v =
  let f :: Nested.Ranked 1 r -> Nested.Ranked 0 Int
...
tindexZSSlow shn (Concrete v) ix | Dict <- eltDictRep (knownSTK @x) =
  if ixInBoundsS (shsTakeIx @shm @shn Proxy ix (Nested.sshape v)) ix
...
{-# INLINE targMinS #-}
targMinS v | sh1@(_ :$$ sh) <- Nested.sshape v =
...
targMaxS
  ...
  => Nested.Shaped (n ': sh) r -> Nested.Shaped (Init (n ': sh)) Int
{-# INLINE targMaxS #-}
targMaxS v | sh1@(_ :$$ sh) <- Nested.sshape v =
  let f :: Nested.Shaped '[m] r -> Nested.Shaped '[] Int
...
tindexZX :: forall shm shn x. KnownSTK x
         => Concrete (TKX2 (shm ++ shn) x) -> IxXOf Concrete shm
         -> Concrete (TKX2 shn x)
{-# INLINE tindexZX #-}
tindexZX (Concrete v) ix | Dict <- eltDictRep (knownSTK @x) =
  if ixInBoundsX (shxTakeIx @shm @shn Proxy ix (Nested.mshape v)) ix
  then Concrete $ Nested.mindexPartial v (fmapUnConcrete ix)
  else case tftkG (STKX (ssxFromShX $ Nested.mshape v) (knownSTK @x)) v of
         FTKX sh x -> tdefTarget (FTKX (shxDropIx @shm @shn ix sh) x)
...
tgatherZX :: forall shm shn shp x. (KnownShX shm, KnownShX shn, KnownSTK x)
          => IShX shm
          -> Concrete (TKX2 (shp ++ shn) x)
          -> (IxXOf Concrete shm -> IxXOf Concrete shp)
...
{-# INLINE targMinX #-}
targMinX v | sh1@(_ :$% sh) <- Nested.mshape v =
...
targMaxX
  ...
  => Nested.Mixed (mn ': sh) r -> Nested.Mixed (Init (mn ': sh)) Int
{-# INLINE targMaxX #-}
targMaxX v | sh1@(_ :$% sh) <- Nested.mshape v =
  let f :: Nested.Mixed '[mm] r -> Nested.Mixed '[] Int
