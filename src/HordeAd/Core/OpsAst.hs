...
instance KnownSpan s => BaseTensor (AstTensor AstMethodLet s) where
  ...
  trfromIntegral a = case ftkAst a of
    FTKR sh' FTKScalar ->
      withShsFromShR sh' $ \(sh :: ShS sh) ->
        astConvUpRFromS sh FTKScalar
        . fromPlain . astFromIntegralS . astPlainPart
        . astConvDownSFromR sh FTKScalar $ a
  trcast a = case ftkAst a of
    FTKR sh' FTKScalar ->
      withShsFromShR sh' $ \(sh :: ShS sh) ->
        astConvUpRFromS sh FTKScalar
    ...
    FTKR sh' x ->
      withShsFromShR sh' $ \(sh :: ShS sh) -> case sh of
        (:$$) @_ @rest _ _ ->
          ...
          astConvUpRFromS (shsInit sh) FTKScalar
  ...
  triota @r n =
    withSNat n $ \snat@(SNat @n) ->
      astConvUpRFromS (snat :$$ ZSS) FTKScalar
            ...
            withShsFromShR shv' $ \shv -> case shv of
              vsnat :$$ restv ->
                case testEquality restu restv of
                  Just Refl ->
                    astConvUpRFromS (snatPlus usnat vsnat :$$ restu) x
        ...
        msnat@(SNat @m) :$$ _ ->
          withSNat i $ \isnat@(SNat @i) -> withSNat n $ \nsnat@(SNat @n) ->
            case cmpNat (snatPlus isnat nsnat) msnat of
              ...
              EQI ->
                astConvUpRFromS (nsnat :$$ shRest) x
    ...
    FTKR sh' x ->
      ...
      withShsFromShR sh2' $ \sh2 ->
        case testEquality (shsProduct sh) (shsProduct sh2) of
          Just Refl -> astConvUpRFromS sh2 x
      ...
      withShsFromShX shpshn0 $ \(shpshn :: ShS shpshn) ->
        ...
        gcastWith (unsafeCoerceRefl
                   :: Take (Rank shp) shpshn ++ Drop (Rank shp) shpshn
                      :~: shpshn) $
        astConvUpXFromS (shm0 `shxAppend` shxDropSSX @_ @shn
    ...
    FTKX @sh' sh' x ->
      withShsFromShX sh' $ \(sh :: ShS sh) -> case sh of
        (:$$) @n @rest _ _ ->
          ...
          astConvUpXFromS (shxInit sh') FTKScalar
    ...
    FTKX @sh' sh' x ->
      withShsFromShX sh' $ \(sh :: ShS sh) -> case sh of
        (:$$) @n @rest _ _ ->
          ...
          . astPlainPart . astConvDownSFromX sh x $ a
  txiota @n @r = astConvUpXFromS (SKnown (SNat @n) :$% ZSX) FTKScalar
                 $ fromPlain $ AstIotaS @n @r SNat
        ...
        withShsFromShX shv' $ \(shv :: ShS shv) ->
          case shxEqual shu' shv' of
            Just Refl ->
              ...
              astConvUpXFromS (smnAddMaybe m' n' :$% shu') x
...
instance KnownSpan s => BaseTensor (AstRaw s) where
    ...
    FTKR sh' x ->
      withShsFromShR sh' $ \(sh :: ShS sh) -> case sh of
        (:$$) @_ @rest _ _ ->
          ...
          -- gcastWith (unsafeCoerceRefl :: Rank sh :~: 1 + Rank (Init sh)) $
          gcastWith (unsafeCoerceRefl :: Rank rest :~: Rank (Init sh)) $
          cAstConvUpRFromS (shsInit sh) FTKScalar
  ...
  triota @r n =
    AstRaw
    $ withSNat n $ \snat@(SNat @n) ->
        cAstConvUpRFromS (snat :$$ ZSS) FTKScalar
        $ fromPlain $ AstIotaS @n @r SNat
          ...
          usnat :$$ restu ->
            withShsFromShR shv' $ \shv -> case shv of
              vsnat :$$ restv ->
                case testEquality restu restv of
                  Just Refl ->
                    cAstConvUpRFromS (snatPlus usnat vsnat :$$ restu) x
          ...
          withSNat i $ \isnat@(SNat @i) -> withSNat n $ \nsnat@(SNat @n) ->
            case cmpNat (snatPlus isnat nsnat) msnat of
              GTI -> error $ "rslice: argument tensor too narrow: "
                             ++ show (i, n, fromSNat' msnat)
              EQI ->
                cAstConvUpRFromS (nsnat :$$ shRest) x
                . AstSliceS isnat nsnat (SNat @(m - (i + n)))
  ...
  trreshape sh2' (AstRaw a) = AstRaw $ case ftkAst a of
    FTKR sh' x ->
      ...
      withShsFromShR sh2' $ \sh2 ->
        case testEquality (shsProduct sh) (shsProduct sh2) of
          Just Refl -> cAstConvUpRFromS sh2 x
...
instance KnownSpan s => ConvertTensor (AstRaw s) where
  ...
  rfromX (AstRaw t) = AstRaw $ case ftkAst t of
    FTKX sh' x ->
      withShsFromShX sh' $ \(sh :: ShS sh) ->
        cAstConvUpRFromS sh x $ cAstConvDownSFromX sh x t
  xfromR (AstRaw t) = AstRaw $ case ftkAst t of
    FTKR shr x ->
      withShsFromShR shr $ \(sh :: ShS sh) ->
        cAstConvUpXFromS (shCastSX knownShX sh) x
        $ cAstConvDownSFromR sh x t
...
astConcreteRaw ftk v = case ftk of
  ...
  FTKR sh' FTKScalar -> AstRaw $
    withShsFromShR sh' $ \sh ->
      withKnownShS sh $
      cAstConvUpRFromS sh FTKScalar $ AstConcreteS $ unConcrete $ sfromR v
  FTKS ZSS FTKScalar ->
  ...
  _ -> concreteTarget
         ...
         (\sh (AstRaw t) -> AstRaw $ cAstConvUpRFromS sh FTKScalar t)
