...
module HordeAd.Core.Ops
  ( -- * The tensor classes and support datatypes
    LetTensor(..), ShareTensor(..), BaseTensor(..), ConvertTensor(..), HFun(..)
    -- * The giga-constraint
...
-- rmatmul2 m1 m2 = rmap1 (rmatvecmul (rtr m2)) m1
  ...
  tsindex0 :: forall sh1 x. (KnownShS sh1, KnownSTK x)
           ...
           -> target (TKS2 '[] x)
  tsindex0 | Refl <- lemAppNil @sh1 = tsindex
  ...
  txindex0 :: forall sh1 x. (KnownShX sh1, KnownSTK x)
           ...
           -> target (TKX2 '[] x)
  txindex0 | Refl <- lemAppNil @sh1 = txindex
