...
evalS !s !c = let (abShared, cShared) = sregister c (astBindings s)
  ...
  FoldS @rm @shm @k p as _df rf x0' as' ->
    let odShn = voidFromShS @r @sh
        domsF = V.fromList [odShn, voidFromShS @rm @shm]
        ...
        crsUnshared = rg cShared  -- not duplicated directly, but just in case
                         (V.fromList
                            ...
                            , DynamicShaped as ])
        domsG = V.fromList [odShn, voidFromShS @rm @(k ': shm)]
        (abShared2, crs) = dregister domsG crsUnshared (astBindings sShared)
        s2 = sShared {astBindings = abShared2}
        (cx0, cas) = domsToPair crs
        s3 = evalS s2 cx0 x0'
  ...
  ScanS @rm @shm @k @sh1 p as _df rf x0' as' ->
    let odShn = voidFromShS @r @sh1
        ...
        domsToPair doms = (sfromD $ doms V.! 0, sfromD $ doms V.! 1)
        domsF3 = V.fromList [odShn, odShn, voidFromShS @rm @shm]
        domsTo3 :: ADReadyS f
...
evalH !s !c = let (abShared, cShared) =
  ...
  MapAccumRR @r @n domB q as _df rf domsOD x0' as' ->
    let width = case V.unsnoc as of
        ...
        domsToPair doms = (rfromD $ doms V.! 0, V.tail doms)
        bLen = V.length domB
        domsF3 = domB V.++ V.cons odShn domsOD
        ...
        rg = rmapAccumR domsOD
                        ...
                        domsF3
        rcrsUnshared =
          rg c0 (mapHVectorRanked11 rreverse crest
                 V.++ V.cons
                        (DynamicRanked $ rreverse q)
                        (mapHVectorRanked11 rreverse as))
        domsG = V.cons odShn (voidFromHVector as)
        (abShared2, rcrs) = dregister domsG rcrsUnshared (astBindings sShared)
        s2 = sShared {astBindings = abShared2}
  ...
  MapAccumRS @k @r @sh1 domB q as _df rf domsOD x0' as' ->
    let odShn = voidFromShS @r @sh1
        ...
        rg = smapAccumR (Proxy @k)
                        ...
                        domsF3
        rcrsUnshared =
          rg c0 (mapHVectorShaped11 @k sreverse crest
                 V.++ V.cons
                        (DynamicShaped $ sreverse q)
                        (mapHVectorShaped11 @k sreverse as))
        domsG = V.cons odShn (voidFromHVector as)
        (abShared2, rcrs) = dregister domsG rcrsUnshared (astBindings sShared)
        s2 = sShared {astBindings = abShared2}
        (cx0, rcas) = domsToPair rcrs
