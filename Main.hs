...
-- * with profiling on (cabal run simplifiedOnlyTest --enable-optimization -w ~/r/ghc.head/_build/stage1/bin/ghc --constraint="ghc-typelits-knownnat==0.7.10" --constraint="ghc-typelits-natnormalise==0.7.9" --ghc-options=-fprof-auto-calls --enable-profiling) the test fails both with and without -fspecialise-aggressively, but no stack trace is printed in either case (even if the exe is run with `+RTS -xc`) and that's because we get `Segmentation fault (core dumped)` very early; the same without `--ghc-options=-fprof-auto-calls`
revShort ::
