...
type InterpretAst a =
  ...
  , forall n. CRanked RealFloat a n
  , forall n. CRankedPrimal EqB a n
  , forall n. CRankedPrimal OrdB a n
  , forall n. CBooleanOfMatches a n
  )
...
{- Needed for GHC <= 9.4, but breaks due to the quantified constraint
-- depending on n (on KnownNat n, specifically), so has to be disabled.
