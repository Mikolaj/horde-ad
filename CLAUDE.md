...
Lint/format: `.hlint.yaml` (hlint with `--cpp-simple -XNoStarIsType`) and `.stylish-haskell.yaml` (stylish-haskell); both are on PATH. Known and accepted hlint blind spot: its parser fails on `import GHC.TypeLits (…, type (*))`, silently skipping every file with such an import, so never report those files as "hlint-clean" — as of 2026-07 seven files, notably `AstSimplify.hs` and `OpsConcrete.hs`, plus `ConvVjpBench.hs` and four example modules (`grep -rl 'type (\*)' --include=*.hs src/ test/ bench/ example/` lists the current set). CI (haskell-ci generated) builds and tests with GHC 9.10.3, 9.12.4 and 9.14.1, but its tests step runs only `minimalTest -p "detSquare"` (plus the benchmark smoke runs) — CAFlessTest and the other suites, including every printed-AST test, are validated only by local runs.
...
- Run `git` writes, `cabal` and `gpg` unsandboxed — the inner sandbox mounts `.git/config`, `~/.cabal` and `~/.gnupg` read-only. `git checkout -b` / `branch -f` fail to lock the config, though the ref often moves anyway — verify refs afterwards. GPG signing (`commit.gpgsign` is on) fails sandboxed and works unsandboxed, so never fall back to `--no-gpg-sign`; SSH keys are likewise unreachable, blocking sandboxed pushes. Other read-only HOME mounts (e.g. `~/.claude/projects`) also need unsandboxed commands for deletions.
